...
describe("install smoke (fresh offline tarball install)", () => {
  ...
  // ponytail: npm strips @scope/ from tarball names; core (@arnilo/prism) -> arnilo-prism-0.2.5.tgz.
  // Regression guard so a future rename can't silently re-mangle the published filename.
  it("core tarball filename is arnilo-prism-0.2.5.tgz (npm strips the @scope/)", () => {
    assert.ok(
      result.tarballNames.includes("arnilo-prism-0.2.5.tgz"),
      `expected 'arnilo-prism-0.2.5.tgz' in ${JSON.stringify(result.tarballNames)}`,
    );
    ...
    for (const meta of ["arnilo-prism-providers-0.2.5.tgz", "arnilo-prism-compaction-0.2.5.tgz", "arnilo-prism-all-0.2.5.tgz"]) {
...
describe("peer-version policy (plan 024 Task 3, Decision A: exact pins)", () => {
  ...
  it("a mismatched exact pin fails clearly with npm ERESOLVE naming the conflicting peer", () => {
    ...
    const coreInstall = run(
      "npm",
      ["install", join(stage, "arnilo-prism-0.2.5.tgz"), "--offline", "--no-audit", "--no-fund", "--no-update-notifier"],
      consumer,
    );
