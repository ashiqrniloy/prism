...
`);
  ...
  // 5. Compose every 0.0.14 optional capability family from packed public imports.
...
console.log("PACKED 0.0.14 COMPOSITION OK");
`);
...
describe("install smoke (fresh offline tarball install)", () => {
  ...
  it("packed 0.0.14 optional capabilities compose through public imports", () => {
  ...
  // ponytail: npm strips @scope/ from tarball names; core (@arnilo/prism) -> arnilo-prism-0.0.14.tgz.
  // Regression guard so a future rename can't silently re-mangle the published filename.
  it("core tarball filename is arnilo-prism-0.0.14.tgz (npm strips the @scope/)", () => {
    assert.ok(
      result.tarballNames.includes("arnilo-prism-0.0.14.tgz"),
      `expected 'arnilo-prism-0.0.14.tgz' in ${JSON.stringify(result.tarballNames)}`,
    );
    ...
    for (const meta of ["arnilo-prism-providers-0.0.14.tgz", "arnilo-prism-compaction-0.0.14.tgz", "arnilo-prism-all-0.0.14.tgz"]) {
