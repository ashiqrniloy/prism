...
`,
  ...
  // 5. Compose every 0.1.0 optional capability family from packed public imports.
  writeFileSync(
...
console.log("PACKED 0.1.0 COMPOSITION OK");
`,
  );
...
describe("install smoke (fresh offline tarball install)", () => {
  ...
  it("packed 0.1.0 optional capabilities compose through public imports", () => {
  ...
  // ponytail: npm strips @scope/ from tarball names; core (@arnilo/prism) -> arnilo-prism-0.1.0.tgz.
  // Regression guard so a future rename can't silently re-mangle the published filename.
  it("core tarball filename is arnilo-prism-0.1.0.tgz (npm strips the @scope/)", () => {
    assert.ok(
      result.tarballNames.includes("arnilo-prism-0.1.0.tgz"),
      `expected 'arnilo-prism-0.1.0.tgz' in ${JSON.stringify(result.tarballNames)}`,
    );
    ...
    for (const meta of ["arnilo-prism-providers-0.1.0.tgz", "arnilo-prism-compaction-0.1.0.tgz", "arnilo-prism-all-0.1.0.tgz"]) {
