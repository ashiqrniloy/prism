...
function canaryEnv() {
  return {
    PRISM_LIVE_CANARIES: "1",
    PRISM_CANARY_TIMEOUT_MS: "1000",
    PRISM_CANARY_PROVIDER_URL: "https://provider.example/v1/chat/completions",
    PRISM_CANARY_PROVIDER_API_KEY: "provider-canary",
    PRISM_CANARY_PROVIDER_MODEL: "model",
    PRISM_CANARY_MCP_URL: "https://mcp.example/rpc",
    PRISM_CANARY_MCP_TOKEN: "mcp-canary",
    PRISM_CANARY_A2A_URL: "https://a2a.example/rpc",
    PRISM_CANARY_A2A_TOKEN: "a2a-canary",
...
test("SPDX policy allows frozen licenses and rejects prohibited or missing licenses", () => {
  ...
  assert.deepEqual(
    verifySbom(
      { spdxVersion: "SPDX-2.3", packages: [{ name: "@ag-ui/core", versionInfo: "0.0.57", licenseDeclared: "NOASSERTION" }] },
      policy,
    ),
    { packages: 1, licenses: 1 },
  );
  assert.throws(
    () =>
      verifySbom(
        { spdxVersion: "SPDX-2.3", packages: [{ name: "@ag-ui/core", versionInfo: "0.0.58", licenseDeclared: "NOASSERTION" }] },
        policy,
      ),
    /NOASSERTION/,
  );
...
test("source/artifact scanner detects representative credentials without echoing them", async () => {
  ...
  await assert.rejects(
    () => scanSecrets([root]),
    (error: Error) => error.message.includes("aws-access-key") && !error.message.includes(canary),
  );
});
...
test("live canaries skip without gate and emit bounded credential-free aggregate results", async () => {
  ...
  await assert.rejects(
    () => runCanaries({ env: { PRISM_LIVE_CANARIES: "1" }, fetcher: async () => assert.fail("network called") }),
  ...
  assert.deepEqual(
    report.results.map((item: { kind: string }) => item.kind),
    ["provider", "mcp", "a2a", "web"],
  );
  const serialized = JSON.stringify(report);
  for (const secret of ["provider-canary", "mcp-canary", "a2a-canary", "brave-canary", "opaque-session"])
...
test("live canary failures and timeout are bounded and redacted", async () => {
  const env = canaryEnv();
  await assert.rejects(
    () => runCanaries({ env, fetcher: async () => new Response("secret body provider-canary", { status: 401 }) }),
    (error: Error) => /HTTP 401/.test(error.message) && !error.message.includes("provider-canary"),
  );
  await assert.rejects(
    () =>
      runCanaries({
...
test("security workflows pin actions, isolate live secrets, and gate publication", () => {
  ...
  for (const [index, workflow] of workflows.entries()) {
    ...
    for (const uses of workflow.matchAll(/uses:\s*([^\s#]+)/g))
      assert.match(uses[1]!, /@[a-f0-9]{40}$/, `${files[index]} action is not immutable: ${uses[1]}`);
  }
