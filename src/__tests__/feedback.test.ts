...
describe("run feedback", () => {
  ...
  test("redacts and freezes bounded immutable records", async () => {
    ...
    assert.throws(() => {
      (record.metadata?.nested as { value: string }).value = "mutate";
    });
  });
  ...
  test("rejects missing/cross-owned runs, invalid bounds, duplicates, and empty input", async () => {
    ...
    await assert.rejects(
      store.append({ id: "missing", runId: "nope", rating: 1, ...owner }),
      (error: unknown) => error instanceof RunFeedbackError && error.code === "ERR_PRISM_RUN_FEEDBACK_RUN_NOT_FOUND",
    );
  ...
  test("filters, orders, paginates, deletes, and honors abort", async () => {
    ...
    assert.deepEqual(
      (await store.query({ ...owner, order: "desc" })).items.map((item) => item.id),
      ["second", "first"],
    );
