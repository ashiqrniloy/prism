...
describe("secure agent composition", () => {
  it("rejects missing schemas, duplicate tools, and incomplete security inputs", () => {
    assert.throws(
      () => createSecureAgent(options({ tools: [{ name: "read", execute: () => ({ toolCallId: "call", name: "read" }) }] }) as never),
      /parameters schema/,
    );
    assert.throws(
      () =>
  ...
  it("preserves secure defaults while allowing a narrower run limit", async () => {
    ...
    await assert.rejects(() => agent.createSession().run("go", { redactor: createSecretRedactor(["other"]) }), AgentRunStateError);
        ...
        tools: [
          {
            ...
            execute: () => {
              calls += 1;
              return { toolCallId: "call-1", name: "read", value: "ok" };
            },
          },
        ],
