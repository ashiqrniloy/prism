...
describe("identity contracts", () => {
  ...
  it("identityTelemetryAttributes expose refs without credential secret material", () => {
    const attrs = identityTelemetryAttributes(verifiedIdentity({
  ...
  it("enforces frozen identity byte/field caps", () => {
    ...
    assert.throws(
      () => assertIdentityActive(verifiedIdentity({ scopes: Array.from({ length: DEFAULT_IDENTITY_LIMITS.maxScopes + 1 }, (_, i) => `s${i}`) })),
  ...
  it("IdentityVerifier host seam and resolveRunIdentity reject ownership conflicts; tools see identity", async () => {
    ...
    const agent = createAgent({
      ...
      provider: { id: "mock", async *generate() { yield providerTextDelta("ok"); yield providerDone(); } },
