...
const agent = core.createAgent({
  ...
  provider: {
    id: "mock",
    async *generate() {
      yield core.providerTextDelta("ok");
      yield core.providerDone();
    },
  },
});
const handler = agUi.createAgUiHandler({
...
await measure("ag-ui-handler", "web-in-process", async (index) => {
  const response = await handler(
    new Request("https://bench.test/ag-ui", {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        context: [],
        forwardedProps: {},
        messages: [{ id: `message-${index}`, role: "user", content: "hello" }],
      }),
    }),
  );
...
const records = [
  {
    ...
    redacted: true,
    event: { type: "agent_started", sessionId: "bench-session", runId: "stored-run" },
  },
  {
    id: "event-finish",
    sessionId: "bench-session",
    ...
    redacted: true,
    event: { type: "agent_finished", sessionId: "bench-session", runId: "stored-run" },
  },
];
const replay = agUi.createPersistenceAgUiReplay(
  { queryEvents: async () => ({ items: records }) },
  {
    resolveRun: () => ({ ref: { sessionId: "bench-session", runId: "stored-run" } }),
    ownership: (authorization) => authorization.ownership,
  },
);
const replayHandler = agUi.createAgUiHandler({
  authorize: () => ({ ownership: { userId: "bench" } }),
  sessionFactory: () => {
    throw new Error("terminal replay must not start");
  },
  replay,
});
await measure("ag-ui-replay", "memory-page", async () => {
  const response = await replayHandler(
    new Request("https://bench.test/ag-ui?cursor=cursor", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        ...
        context: [],
        forwardedProps: {},
        messages: [{ id: "message", role: "user", content: "replay" }],
      }),
    }),
  );
  const output = await response.text();
