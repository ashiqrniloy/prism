...
export async function runCanaries({ env = process.env, fetcher = fetch } = {}) {
  ...
  const { captured: sessionId, ...mcpResponse } = await request(fetcher, "mcp", mcpUrl, { method: "POST", headers: mcpHeaders, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2025-11-25", capabilities: {}, clientInfo: { name: "prism-release-canary", version: "0.0.8" } } }) }, timeoutMs, "mcp-session-id");
