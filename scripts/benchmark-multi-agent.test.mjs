...
describe("multi-agent runtime coverage and baselines", () => {
  it("inventories all 59 manifests in the evidence matrix", () => {
    const names = workspaceManifests();
    assert.equal(names.length, 59, `expected 59 manifests, found ${names.length}`);
    ...
    assert.equal(rows.length, 59, "evidence table has 59 package rows");
