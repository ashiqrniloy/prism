...
test("baseline manifest count is coherent with the real filesystem (0.2.1 adds no package)", () => {
  ...
  const hasProviderFamily = packages.has("prism-providers");
...
test("phase21-freeze.test.mjs is retired from npm test (plan 057) but stays runnable standalone", () => {
  assert.ok(
    !effectiveTestChain().includes("scripts/phase21-freeze.test.mjs"),
    "retired freeze gate must not run in npm test (plan 057); run standalone for audits",
  );
  assert.ok(!effectiveTestChain().includes("scripts/phase20-freeze.test.mjs"), "phase20 freeze test retired too (plan 057)");
});

test("phase 21 baseline was captured at or after the phase 20 baseline (content dates, not mtime)", () => {
  ...
  const previous = JSON.parse(readFileSync(url("./phase20-baseline.json"), "utf8")).captured;
  assert.ok(
  ...
  assert.ok(
    ...
    `phase 21 baseline captured ${baseline.captured} predates the phase 20 baseline ${previous}`,
  );
});
