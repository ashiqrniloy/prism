...
Current **10** publishable manifests (root + 9 workspace packages):

`@arnilo/prism`, `@arnilo/prism-acp-agent`, `@arnilo/prism-ag-ui`
...
`@arnilo/prism-providers/clinepass`, `@arnilo/prism-providers/commandcode`, `@arnilo/prism-providers/deepseek`, `@arnilo/prism-providers/google`, `@arnilo/prism-providers/hyper`, `@arnilo/prism-providers/kimi`, `@arnilo/prism-providers/neuralwatt`, `@arnilo/prism-providers/ollama`
...
| Install core + all 19 provider adapters | `npm install @arnilo/prism @arnilo/prism-providers` (import `@arnilo/prism-providers/<adapter>`) |
...
| Providers | all 19 `@arnilo/prism-providers/<adapter>` subpaths plus the OpenAI-compatible transport | Per-package conformance suites in the default network-free `npm test`; live canaries stay credential-gated (`PRISM_LIVE_PROVIDER_TESTS=1`). |
