...
First-party workspace packages (each has non-optional `@arnilo/prism@0.0.21` peer and `sideEffects: false`; RAG also peers on memory, and server also peers on workflows):
...
`@arnilo/prism-ag-ui` is a publishable optional code package with root AG-UI exports and stable `./acp` sibling, peer `@arnilo/prism@0.0.21`, pinned `@ag-ui/core@0.0.57` / `@agentclientprotocol/sdk@1.3.0`, and no import-time network/listener/run. It is included by `@arnilo/prism-all` only—not `@arnilo/prism-code` or `@arnilo/prism-sdk`—so coding and SDK profiles stay free of UI protocol dependencies.
...
| Validate clean tag/version/ranges and reject registry collisions | `npm run release:check -- --version 0.0.21` |
| Preview deterministic publish order | `npm run release:publish -- --version 0.0.21 --dry-run --allow-dirty --allow-untagged` |
| Resume interrupted tagged publication | `npm run release:publish -- --version 0.0.21 --resume --report release-artifacts/publish-report.json` |
...
- **Tarball filenames.** npm strips the `@scope/` prefix, so the core package `@arnilo/prism` produces a tarball named `arnilo-prism-0.0.21.tgz`; first-party packages produce `arnilo-prism-provider-<name>-0.0.21.tgz` / `arnilo-prism-compaction-<name>-0.0.21.tgz` / `arnilo-prism-coding-agent-0.0.21.tgz`; family/profile packages produce `arnilo-prism-{providers,compaction,base,code,sdk,all}-0.0.21.tgz`. The CLI bin name `prism` is unaffected by the package name (`npx prism` still works; npm allows the bin field to differ from the package name).
...
```json
{
  ...
  "dependencies": {
    "@arnilo/prism": "0.0.21",
    "@arnilo/prism-provider-openai": "0.0.21",
    "@arnilo/prism-compaction-observational-memory": "0.0.21"
  }
}
```
...
Release publication derives all **44** manifests from the workspace once, validates exact `0.0.21` manifest/lockfile/internal ranges, then uses deterministic dependency order. `release:check` requires a clean commit tagged `v0.0.21` and rejects any existing registry version. `release:publish --resume` skips only registry versions whose internal dependency fingerprint matches the local manifest; conflicting versions fail closed. Each attempted package is written immediately to the JSON report, so a failed job can rerun safely. `--dry-run` performs registry availability checks and invokes `npm publish --dry-run` with explicit public access, provenance, and `latest` tag, but does not publish.

```bash
npm run release:check -- --version 0.0.21
npm run release:publish -- --version 0.0.21 --dry-run --allow-dirty --allow-untagged
```
...
- **Required `@arnilo/prism` peer.** Every first-party code package declares a non-optional `@arnilo/prism@0.0.21` peer (`peerDependenciesMeta` must not mark `@arnilo/prism` optional; other peers such as `playwright-core` may be optional). The range stays pinned to `0.0.21` for the current 0.x release and will widen to `^1.0.0` at the 1.x stable release. Inside the workspace each package also declares `"@arnilo/prism": "file:../.."` in `devDependencies` so `npm install` resolves the peer locally; that devDependency is stripped from consumer installs and is not a runtime dependency.
