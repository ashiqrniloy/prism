...
- [x] Task 10: Release 0.10.0 — CHANGELOG, publish, post-publish verification, plan/roadmap bookkeeping
  ...
  - Completion notes:
    ...
    - Publish + registry smoke: **not executed, and not executable here** — `npm whoami` returns `ENEEDAUTH` (no credentials on this machine) and npm provenance needs CI (`id-token: write`), which is exactly the 099 Task 4 precedent: the cut ends at an operator handoff. Registry state today: `@arnilo/prism` latest is **0.9.0**, `@arnilo/prism-hooks` is 404. Operator steps: commit the cut, `git tag v0.10.0`, push branch + tag, watch the publish job, then `npm run post-publish:smoke`.
...
- **The cut is release-prepared, not published.** `npm whoami` fails with `ENEEDAUTH` on this machine, and the local `release.mjs publish` path cannot produce the provenance attestation every previous cut carried (it needs `id-token: write` in CI), so Task 10 stops at the operator handoff: preflights, changelog, migration notes, roadmap/index bookkeeping, and a runnable post-publish smoke. Task 4's acceptance ("all publishable manifests publish; post-publish verification passes") is therefore **unmet by this session** — by construction, since registry writes are operator-authorized in this repo, and the alternative (a local token publish without provenance) would have been a regression against 0.8.0/0.9.0.
...
- **Gate suites flake under load.** One `npm test` gate-suites run failed at 36.5 s (versus 38.9 s on the green run) with no failing test name in the captured tail; the identical `GATE_FILES` list passed standalone (0 fail) and the next two full runs were green. This is the same class as 099's recorded `run-bundle` perf flake: benchmark/sanity assertions inside the gate stage are load-sensitive, and a release chain that can fail on machine noise is a release blocker. Capture the failing test name in the runner (or gate perf assertions behind `isMachineLoaded()`) so the next flake is diagnosable instead of anecdotal. Priority: medium (release-blocking false negative, no known real regression).
