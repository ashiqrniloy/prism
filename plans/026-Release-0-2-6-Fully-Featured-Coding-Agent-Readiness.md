...
- [x] Task 4 — Demand-gated forge breadth (GitLab or Bitbucket only)
  - Completed 2026-08-16 (deferred via demand gate): no named consumer recorded, so no adapter source, docs page, or export was created. The phase26 freeze manifest demand registry keeps `gitlab-forge` and `bitbucket-forge` at `status: deferred` with `consumer: null`; `packages/coding-agent/src/forge/gitlab.ts` and `bitbucket.ts` remain absent and the forge barrel exports no GitLab/Bitbucket adapter (enforced by the freeze test). Deferral recorded in `docs/forge-integration.md` (demand-gated breadth section) and the coding-agent CHANGELOG; activation criteria (named consumer + exactly one thin adapter against `ForgeOperations`) documented in the manifest entry. No source edited, no deviation recorded.
  - Acceptance Criteria:
...
- [x] Task 5 — Durable ACP/live-task and managed-process recovery across restarts and replicas
  - Completed 2026-08-16: metadata-only durable recovery over `CheckpointStore` CAS + `LeaseStore` fencing.
  - Process side (`packages/coding-agent/src/process/recovery.ts` + sessions.ts): optional `checkpoints`+`leases`+`ownerId` (all three together; partial config fails closed at construction) persist intent BEFORE spawn into `prism.coding-agent.process.v1` (schemaVersion 1, category `coding-process`) and CAS/fence-write every lifecycle transition under per-record leases (30 s/300 s TTL, monotonic fencing tokens, serialized per-record write chains so slow stores never invert CAS order). `recover()` is attach-if-attested via an optional host `recoveryBackend` (opaque non-secret `ref` on PTY/sandbox handles, bounded attach timeout 30 s/120 s); starting/running records otherwise atomically become `unknown` with exitCode null — no fabricated exit, no PID probing, no duplicate spawn; two replicas cannot both own/attach/mutate a process (held-lease skip + CAS/fence rejection); `cancelOwned` after recovery reaches the attached backend or records unknown. Records 32/128 cap with oldest-terminal eviction; corrupt/forbidden/cross-tenant records fail closed; new errors `ERR_PRISM_RECOVERY_*` (unsupported/limit/ownership/fence/unknown/untrusted/timeout).
  - ACP side (ag-ui): `PersistedAcpSession.activeRun` additive optional bounded ref (frozen 512-byte cap) persisted on live runs (running/suspended/terminal + durable version), restored on restart; `createAcpRunRecovery` re-resolves refs against `AgentRunLifecycle.status` (suspended → pending approval ids, terminal → terminal, unprovable in-flight → unknown, never a restarted prompt) and provides durable cancellation (`prism.coding-agent.cancel.v1` markers, CAS + lease fenced, ownership/version/fence checked, terminal/idempotent, never replays pending/dispatched tools); `session/cancel` aborts live controllers (parity) and writes the marker for restored runs.
  - Evidence: `process-recovery.test.ts` 15 tests (crash windows, replica race, unsupported attach → unknown, T5 fabricated-exit, stale fence, cancelOwned both paths, eviction, corrupt records); `acp-recovery.test.ts` 16 tests (ref caps/validation, status mapping incl. unknown, cancel idempotent/terminal/fence/version/ownership, agent persist-restore-cancel roundtrip); protected conformance `scripts/phase26-recovery-conformance.test.mjs` 8/8 green against memory AND real Postgres (PRISM_TEST_POSTGRES_URL, postgres:16 container) with two-replica split-brain, restart-unknown, terminal-before-recovery, and cross-replica cancellation; coding-agent suite 42/42 process, ag-ui 203/203; biome lint 0; no preservedSurface core file touched.
  - Docs: process-sessions.md (recovery section), acp.md, coding-workspaces.md (fencing), migration.md (0.2.5 → 0.2.6 additive decision + downgrade rules), index.md, README/CHANGELOG (coding-agent + ag-ui). Deviation D-T5-1 recorded: Task 0 test paths pointed at nonexistent `process/__tests__`/`acp/__tests__` dirs whose dist output the package test globs never run; both tests live in the real `src/__tests__` dirs (manifest + baseline updated).
  - Acceptance Criteria:
...
- [x] Task 6 — Bounded patch review and incremental LSP/check diagnostics workflow
  - Completed 2026-08-16: bounded patch-review manifest + LSP/check diagnostic normalization and delta over existing primitives.
  - review.ts: `createCodingPatchReviewManifest` builds a bounded manifest (repository/worktree identity with credential-free remote fingerprint, base/head, patch digest/uri/bytes, changed paths, diffstat, check summaries ≤8 KiB each, diagnostic summaries) plus a structural ArtifactService attach input whose `preview.review` embeds the manifest and whose hash is the patch sha256; `assertCodingPatchAccepted` derives pending|accepted|rejected|superseded bound to the exact artifact revision + digest + identity — any patch/repository/worktree/base/head change supersedes prior acceptance, stale acceptances refused (newer revision attached after approval), acceptance never applies/commits/pushes/merges, raw patch bodies never embedded; `CodingPatchReviewError` family ERR_PRISM_REVIEW_INPUT/LIMIT/BINDING/STATE/OWNERSHIP; frozen caps revisions 8/32, diagnostic summaries 500/5000, manifest bytes 64 KiB/256 KiB.
  - diagnostics.ts: `normalizeDiagnostics` (host-parsed check output + LSP results into one bounded shape: containment, non-negative finite positions, severity validation, control-char strip, UTF-8 truncation, per-file caps charge before retention, malformed dropped fail-closed) and `diagnosticDelta` (deterministic added/removed/unchanged via file:source:position:code identity, dedupe, stale-version previous views ignored).
  - LSP stays opt-in (user requirement): `LanguageIntelligence.syncDocument` (full-content didChange, monotonic versions — didOpen stamps 1, each didChange increments) and `.diagnosticDelta` (push cache or pull diagnostics with previousResultId reuse full/unchanged, changed-files-only refresh, generation = document version, stale views dropped per file) extend the standalone host-activated `createLanguageIntelligence` factory; nothing spawns from `createCodingTools`/`createAllTools` and no agent assembly instantiates LSP.
  - Evidence: review.test.ts 12 tests (digest determinism, invalid identity matrix, caps, accepted/rejected/pending/superseded paths incl. stale acceptance and tampered preview, ownership); language-diagnostics.test.ts 11 tests (normalization matrix, delta determinism/dedupe/stale, syncDocument versions + versioned push, pull resultId reuse, opt-in no-spawn); coding-agent suite 386/386; biome lint 0.
  - Docs: coding-review-and-diagnostics.md (new), language-intelligence.md sync/pull/delta section, work-artifacts-and-review.md coding patch composition, index.md entry, coding-agent-tools.md review handoff row, README/CHANGELOG; no deviation recorded.
  - Acceptance Criteria:
...
- [x] Task 7 — Protected real coding-agent release journey
  - Completed 2026-08-16: packed-consumer protected journey over real host services, retained evidence report, and release-evidence wiring.
  - scripts/fixtures/phase26-coding-journey.mjs runs INSIDE a fresh packed consumer (never workspace paths) with one real leg per requirement: provider (real LLM call through the Prism AIProvider contract via a host adapter module, no-secret-leak scan), docker (digest-pinned image via createDockerSandbox, capability metadata asserted, container closed), workspace (real Postgres checkpoints/leases via createPostgresPersistence + createCodingWorkspaceLifecycle: clone, worktree add/lock, verify), agent-edit (real provider-driven edit through createPrismAcpAgent: read-before-write + policy approval via requestPermission allow-once, file changed assert), check-diagnostics (real `node --check` named check + host parser + normalizeDiagnostics + diagnosticDelta added/unchanged), patch-review (git diff -> createCodingPatchReviewManifest -> server createArtifactService attach/approve -> assertCodingPatchAccepted accepted then superseded by a newer revision), process-recovery (two replicas over the same Postgres stores: split-brain fence unknown, fenced cancelOwned no-op with no fabricated exit, lease-lapse restart with attach-if-attested control-only reattach and no re-spawn, terminal durable state after dispose), durable-cancel (rejected approvals never replay the write; createAcpRunRecovery cancel -> cancelled, terminal-idempotent, Postgres marker), forge (real GitHub push + lookup-before-create PR idempotency + check reads + reconcileHandoff + cleanup close PR and delete branch), browser (host Playwright chromium in the consumer, loopback fixture page, snapshot text assert, context closed before host browser), pty (host PTY adapter when in the frozen profile). Every side effect carries the run suffix; cleanup records each step and unknown cleanup blocks.
  - scripts/phase26-coding-journey.test.mjs is the protected gate (never a passing skip): missing PRISM_CODING_JOURNEY or any frozen env/service prints BLOCKED GATE and exits 1; packs 10 packages, verifies exact installed versions, installs pinned playwright-core into the consumer, runs the fixture under the frozen wall caps (20/40 min, cleanup 5/15 min), leak-scans stdout/stderr/report against every credential-looking env value, and regenerates the retained scripts/phase26-coding-journey-report.json (timings/states/ids only).
  - scripts/release-skip-manifest.mjs gained the protected coding journey surface: pass only for a retained pass report; blocked for blocked/partial reports (fail closed); documented protected gap (requiredEnv PRISM_CODING_JOURNEY) for not_run/missing reports.
  - .github/workflows/coding-journey.yml provisions the full protected profile (Postgres service, host Docker + digest-pinned image var, pinned Chromium, provider adapter from repo node_modules + secrets, python3-pty host adapter, forge repo/token) and uploads the report artifact.
  - Deviation D-T7-1 recorded: the packed journey exposed that the Task 6 review helper compared `artifact.artifactId`, but the real server ArtifactRecord carries `id` — review.ts and review.test.ts amended to the real shape (outside task7 allowedFiles).
  - Evidence: local protected run with real Postgres + real Docker + real Playwright chromium + real PTY (python host) + mock provider adapter passed provider/docker/workspace/agent-edit/check-diagnostics/patch-review/process-recovery/durable-cancel/browser/pty (forge blocked locally only by missing real GitHub credentials, as designed); every cleanup step passed; import probe of all journey imports from the packed consumer green; docs updated (coding-security.md, browser-automation.md, release-and-install.md runbook, forge-integration.md cross-link, index.md).
  - Acceptance Criteria:
...
- [x] Task 8 — Documentation finalization, 0.2.6 bump, compatibility review, and exit gate
  - Completed 2026-08-16: docs finalized, 0.2.5 → 0.2.6 bump applied, compatibility reviewed, exit gate green.
  - Docs: root CHANGELOG `[0.2.6] - 2026-08-16` release entry; `docs/0.1.0-readiness.md` current line → 0.2.6 (upgrade path/compat promise/security policy refreshed); `docs/index.md` current **0.2.6** with the plan 026 capability summary; `docs/release-and-install.md` `0.2.6 publish handoff` operator runbook (measured deltas, rollback notes, command list); `docs/migration.md` keeps the `## 0.2.5 → 0.2.6` additive note (three versioned namespaces + optional `activeRun`); roadmap 0.2.6 section all seven items checked (forge marked demand-gated deferred); `plans/README.md` plan 026 row `complete`; package README/CHANGELOG 0.2.6 markers (coding-agent, ag-ui); new plan 026 Task 8 freeze test in `src/__tests__/docs.test.ts` (handoff sections, roadmap completion, migration note, peer pin, tarball names, changelog entry, index current line, plans README).
  - Bump: `node scripts/release.mjs bump --from 0.2.5 --to 0.2.6` across 50 manifests + lockfile + exact peers; version-sensitive tests updated (peer pins ×12 packages, packaging/release/index/cli-provider-add/install-smoke tarball names + fakeVersion 0.2.7, core `version` const, docs.test current-line assert); `scripts/package-truth.json` regenerated (50/49/14/9/26 unchanged, version 0.2.6).
  - Compat: pre-refresh plain gate reviewed — expected deltas only (prism `version` const; ag-ui `forward` onRunRef param + `activeRun` on AcpSessionStore/PersistedAcpSession; coding-agent process/repository type widenings for pty/index/recovery/review); zero removals; `--update-baseline` without `--allow-break` (197 insertions / 34 deletions across 3 baselines); post-refresh plain gate green (0 breaking deltas, 50 packages).
  - Gates: npm test exit 0 (3716 tests / 3683 pass / 33 skip / 0 fail incl. phase26-freeze 11/11 + index benchmark); test:coverage exit 0 (core 91.43/84.80/91.60, postgres durable row 24.22 denominator proof, phase23-skip-manifest 6/6 with exitGate counts + protectedEvidence); security:threat-suites 50/50; test:postgres 91/91; phase26-recovery-conformance 8/8 (memory + real Postgres two-replica); phase26-pty-protected 4/4 (real PTY host); typecheck/lint/format 0; pack:dry-run 50/50 twice byte-identical; audit 0 moderate; secret scan 0 findings (1659 files); Node v20.20.2 packed imports 14/14; release evidence 59 surfaces zero blocked; release:gate 0.2.6 green. budgets.json re-baselined for the real root-tarball growth (docs/version literals: packed 847137, unpacked 2922323, files 359) with a dated $comment per convention; the Task 6 flaky push-delta test got the same waitFor guard as its sibling (load-sensitive ceiling documented).
  - Evidence: `scripts/phase26-baseline.json` `exitGate` recorded green (counts, coverage, compat, measurements, protected, blocked:false, version 0.2.6); freeze test done-state passes; exit-gate test conditionalized on task8 for the Task 0 assertion (freeze-test-only edit, no policy change). Protected real coding journey remains a documented protected gap at Task 8 (report `not_run`; real run gated in `.github/workflows/coding-journey.yml` with real provider/Docker/Playwright/GitHub/Postgres services). Publication remains the operator handoff (signed `v0.2.6` tag + npm OIDC trusted publishing + build attestations — never automatic).
  - Acceptance Criteria:
...
- The protected real coding journey (`PRISM_CODING_JOURNEY`) could not run locally at the exit gate: it requires real GitHub forge credentials, a live provider API key, a digest-pinned Docker image, and a host Playwright browser. The retained report is committed as an honest `not_run` placeholder and the release-evidence surface records a **protected** gap (never a passing skip); the real run is wired to `.github/workflows/coding-journey.yml` (scheduled + manual) with every frozen env/service provisioned, and publication is blocked until the operator records a pass or explicitly accepts the documented gap (per the 0.2.3 skip-manifest policy, a protected row with a required env is a documented gap, not a green skip). Forge and provider legs are likewise CI-only by design.
- The phase23 `live-canary-not-pass` and `protected-named` local assertions depend on the baseline `exitGate` counts + `protectedEvidence`; they were recorded as part of this task's evidence (they failed locally before Task 8 only because the baseline was still null — not a product regression).
- Root tarball growth (+6.7 KiB packed vs 0.2.5) from the 0.2.6 release docs and version literals required a `scripts/budgets.json` re-baseline with a dated $comment (recorded release convention; core runtime imports unchanged).
- `src/__tests__/language-diagnostics.test.ts` push-delta test received the same async-publish `waitFor` guard as its sibling (load-sensitive; ceiling documented in the test) — test-only stabilization, no behavior change.
- The phase26-freeze.test.mjs Task 0 assertion `exitGate null until Task 8` was conditionalized on the task8 token so the freeze test can express the post-exit state; no policy/cap/API change.

## Further Actions

- Run the protected real coding journey in CI (`coding-journey.yml`), commit the retained pass report, and record the operator handoff (signed `v0.2.6` tag + npm OIDC trusted publishing + GitHub build attestations on tarballs/SBOM) — publication is never automatic.
- 0.3.0 carries the live-service matrix (NATS real legs, live canaries, provider live tests) that the 0.2.x exit gates keep protected; ERP readiness (roadmap 0.2.7) remains blocked until that matrix is recorded.
- Forge breadth stays demand-gated: GitLab/Bitbucket adapters ship only when a named consumer/date/use case is recorded in the demand registry (priority: medium, no current consumer).
- Future phases should run `npm run format` before committing task code (several Task 1-7 files were formatted only at the Task 8 exit gate).
