...
- [x] Bump and publish only packages changed from Plan 035 onward (2026-08-29: complete — 30-package cut validated; publication = operator tag push)
  - Acceptance Criteria:
...
- Task 8 (complete): committed the reviewed plan 035-039 work (prerequisite — release detection is git-diff based), then generated the final changed set with `release.mjs changed --baseline c600eaa` (30 packages: root + 28 changed → 0.3.1 via per-package `bump --package --type patch`; rag 0.3.1→0.3.2; obscura publishes new at its reviewed initial 0.3.0). Current-line truth advanced (index/readiness `0.3.1`, root `version` export, release page tarball names/peer statements, phase30/27 freeze root-version windows); phase34 version freeze rewritten to derive expected versions from the baseline via the same release.mjs logic (superseded 034-era literals). Peer policy decision: root peers `^0.3.1` on the republished set, `^0.3.0` kept byte-identical on unchanged packages — phase24-truth + packaging gates accept the Decision B window. Compat baselines regenerated via `--update-baseline` (version literal + additive exports only; no `--allow-break`); lockfile + package truth regenerated; per-package changelog sections added (0.3.1/0.3.2, 2026-08-29). Verification on the final clean commit: `check --independent` 30/30 available, `publish --dry-run` 30/30 in dependency order (report `docs/_evidence/phase39-publish-dry-run.json`), pack deterministic, full npm test green (one load-flake re-run green), biome/format clean. 30 annotated package tags cut on HEAD. **Publication is the operator handoff**: push the tags; `release.yml` publishes with OIDC provenance — this task does not publish. Note: two mid-flight cosmetic events (biome reformat of coding-security egress; a concurrently added plan 049) were reverted/absorbed to keep the cut set exactly the reviewed 30.
## Further Actions
