...
Status: complete. Tasks 1–9 completed **2026-09-15/16** against the 0.7.0 working tree; **package boundary changed 2026-09-16 by user direction** — Task 4 moved the implementation out of `@arnilo/prism-core` into the new `@arnilo/prism-channels` package, and Tasks 5/7 added its adapters: the primitive/threat-model review (with executable evidence in [docs/history/079-messaging-primitive-review.md](../docs/history/079-messaging-primitive-review.md)), the transport-neutral channel runtime at `@arnilo/prism-channels` ([docs/messaging-channels.md](../docs/messaging-channels.md)) and the durable journal ([docs/messaging-channel-operations.md](../docs/messaging-channel-operations.md)) — 66 executable tests across core primitives, runtime, journal state, restart recovery, Telegram, experimental Signal, sqlite host end-to-end and SQLite/PostgreSQL conformance. Telegram polling/webhook ingress, durable approval controls, the experimental pinned signal-cli Signal adapter, host examples/fault injection and packaging/current-contract verification are implemented. Release vehicle: **0.8.0** — **moved off the 0.7.0 line on 2026-09-15 by user request** so the 0.7.0 cut stopped waiting on it. It had been assigned to 0.7.0 on 2026-09-14; 0.7.0 shipped on 2026-09-15 without it (six-plan cut: 072, 073, 074, 075, 077, 078), and no channel adapter exists in 0.7.0. The deviation is recorded in [073](073-Release-0-7-0-Host-Completeness.md) “Compromises Made”, [roadmap.md](../roadmap.md) and [plans/README.md](README.md).
...
**Boundary change (2026-09-16, user-directed):** channels move out of `@arnilo/prism-core` into a new first-party package, **`@arnilo/prism-channels`** (`packages/prism-channels`). Reason: channel support will grow (more platforms, each with its own client, peers and release risk) and platform adapters do not belong in the core runtime's release cadence, export budget or peer set. Task 4 extracts what Tasks 2–3 already built and deletes the prism-core module; Tasks 5–9 add adapters and controls inside the new package. The symbol names frozen by Task 1 stay frozen — only the package specifier changes.

| Package surface (symbols frozen by Task 1; package decided 2026-09-16) | Responsibility |
| --- | --- |
| `@arnilo/prism-channels` | Small transport-neutral channel contracts, authorization/binding checks, Prism execution adapter, durable bindings/journal/pairing, bounded delivery state and lifecycle. |
| `@arnilo/prism-channels/telegram` | Official Bot API adapter: long polling, separately mountable Web `Request`/`Response` webhook handler, text sends, callback controls. |
| `@arnilo/prism-channels/signal` | Node-only client for a host-selected signal-cli Unix socket; bounded JSON-RPC requests and receive notifications. |

Package rules: `@arnilo/prism` is the peer dependency and the only way channel code reaches Prism primitives (`Agent`, `AgentSession`, identity/ownership, `CheckpointStore`, `LeaseStore`, redaction) — public entry only, never a `@arnilo/prism-core` internal path; `@arnilo/prism-core` may appear at most as a **dev** dependency for the SQLite/PostgreSQL store legs in tests; the package must build, pack and start without any other family package. Channel-specific types stay here. Do not add a model provider, extension registry, framework server, or mandatory database driver. Telegram and Signal are messaging transports, not `AIProvider` implementations. A further platform is a new subpath in this package, not a new package, unless it brings its own incompatible peer set.
...
Channel code lives at `packages/prism-channels/src/`; the evidence rows below describe Prism core primitives and remain valid after Task 4's move.
...
### Task 1 gate resolutions (2026-09-15, executable evidence in `docs/history/079-messaging-primitive-review.md`)

1. **Conversation/secure-agent:** proven conflict; MVP default is a direct `AgentSession` (`runBoundTurn` compiles and runs). `ConversationService.continue()` cannot compose with a secure agent — it forces `ownership` + `redactor`, which `executeRun` rejects. Not fixed here; a generic opt-out would be its own scoped change.
2. **Idempotency:** `idempotencyKey` dedupes an exact append retry at the same parent only; at a new tip the provider runs again. Admission must claim the operation by checkpoint CAS before `session.run`.
3. **Resume identity/cancellation:** `lifecycle.resumeStream` is the channel resume path (bounded subscription, cancellation reaches the in-flight provider turn, early return aborts). `createAgentRunLifecycle.resume()` drops `signal` — a demonstrated generic gap with a working alternative, kept as a conditional Task 2 item (`AgentRunResumeOptions.signal`).
4. **Final-output provenance:** `buildRunResult` is history-derived; failed runs can carry the previous turn's text and suspended/aborted runs expose empty text. Send only the current operation's terminal `succeeded` output.
5. **Persistence fit:** checkpoint keys are scope-free (`namespace + key`; a foreign scope throws rather than misses), list pagination is numeric OFFSET (not snapshot-stable), and no multi-record transaction exists. Channel keys must embed tenant/user; recovery resolves explicit keys and orders writes; 128 KiB record cap under the memory store's 1 MiB value ceiling.
6. **Version truth:** reviewed revision is 0.7.0 (plan text still says 0.6.0 planning baseline); peers `better-sqlite3 ^13.0.3`, `pg ^8.23.0`; root has zero runtime dependencies.
...
**Frozen API vocabulary** (Task 1 names; Task 2 implemented shape below, durable options land in Task 3):
...
createMessagingRuntime({
  ...
  resolveBinding,   // optional; defaults to a deterministic per-binding session id
  redactor,         // optional; applied to error/notice text before delivery
  limits,
  // Task 3 adds: checkpoints, leases (durable bindings, admission claim, reply journal)
});
...
Task 1 froze these symbol names and the three-part surface split; the 2026-09-16 boundary change moved the specifier from `@arnilo/prism-core/integrations/channels` to `@arnilo/prism-channels` (Task 4) without renaming anything.

The exact small lifecycle/adapter shape was frozen by Task 1: `createMessagingRuntime` returns a runtime with `admit`/`drain`/`diagnostics`/`stop`; adapters expose `connectionId`, `capabilities`, `start(receive)`, `send`, `stop`, and adapters await durable `admit` before transport acknowledgment (Signal has no acknowledgment step). See the review record for the frozen symbol list. Do not create both custom transport and custom execution plugin frameworks: transport varies twice; execution is the existing Prism runtime.
...
- [x] **Task 1 — Primitive, compatibility and threat-model review**
  ...
  - Approach:
    ...
    - Files to Create/Edit: `docs/history/079-messaging-primitive-review.md`; this plan; `packages/prism-core/src/__tests__/messaging-primitives.test.ts` (retained — 7 executable proofs, run via the explicit command in the Outcome below); `scripts/budgets.json` (measured root-tarball rebaseline for this mandated history doc plus unabsorbed post-0.7.0-cut growth).
    - Task 1 Outcome (2026-09-15): all acceptance criteria met at the 0.7.0 revision. 7/7 probes pass (`node --test "packages/prism-core/dist/integrations/channels/__tests__/primitives.test.js"`). Frozen defaults: direct `AgentSession` execution; approval resume via `AgentRunLifecycle.resumeStream`; operation CAS claim before `session.run`; current-run-only output projection; `prism.channels.*` keys embed tenant/user; Signal stays experimental behind the operator acceptable-use gate. Demonstrated generic gap kept for Task 2: `AgentRunLifecycle.resume` does not thread its `signal` (use `resumeStream`; optional one-field core fix). Probe evidence: two objects on one session ID run concurrently (no core cross-object lock); secure override rejected directly and through `createConversationService` (zero provider calls); resume refuses ownership/revision/agentId/fingerprint drift and dispatches an approved tool once; aborted resume reaches the provider turn, persists `aborted`, does not roll back the approved dispatch; failed run `result.text` is the previous turn's answer. Budgets: root packed/unpacked/fileCount rebaselined 2026-09-15 to measured 1247731/4137527/489 with the split recorded in `scripts/budgets.json`; Task 9 re-verifies. No new package, provider, dependency, listener, or background work. Topology caveat: the prism-core `test` script passes `dist/**/__tests__/*.test.js` through the shell, where `**` collapses to one level, so the aggregate `npm test` workspace stage currently runs only `dist/runtime/__tests__/*.test.js` and does not reach nested suites (this review's probe, `runtime/server`, `sessions/*`, `governance/*`) — pre-existing, not introduced here. Task 9 must run the probe's explicit command; whether to fix the package test glob repo-wide is logged in Further Actions.
...
- [x] **Task 2 — Channel contracts, authorization and Prism execution adapter**
  - Acceptance Criteria:
  ...
  - Approach:
    ...
    - Files to Create/Edit: `packages/prism-core/src/integrations/channels/{index,types,limits,runtime}.ts`; `packages/prism-core/src/integrations/channels/__tests__/runtime.test.ts`; `packages/prism-core/package.json`; `docs/messaging-channels.md`; `docs/core.md`; `docs/index.md`; `scripts/budgets.json` (export ceiling); `scripts/package-truth.mjs` + regenerated `scripts/package-truth.json`/`README.md`/`docs/release-and-install.md`; `scripts/phase54-package-map.mjs` + regenerated `docs/_evidence/phase54-package-map.md`. Conditional only (not needed — no core gap required): `src/agent-run-lifecycle.ts`, `src/contracts-protocol.ts`, `src/__tests__/agent-run-state.test.ts`, or `packages/prism-core/src/runtime/server/conversations.ts` and its existing tests if a generic compatibility fix is selected.
    - Task 2 Outcome (2026-09-15): all acceptance criteria met. 14/14 tests pass (`node --test "packages/prism-core/dist/integrations/channels/__tests__/runtime.test.js"`). Implemented: `createMessagingRuntime` with `admit`/`drain`/`diagnostics`/`stop`; host `authorize`/`resolveAgent`/`deliver` seams plus optional `resolveBinding` (deterministic `chan-<sha256 prefix>` session id derived from connection + conversation + thread + actor + ownership + alias + generation, so tenant/connection/actor isolation is structural); deny-by-default denials for malformed, oversized, rejected, revoked, authorization-failed, capacity, stopped and awaiting-decision cases with zero provider/tool calls; `assertIdentityActive` re-check before provider work and before every send; runs pass `identity` (never a per-run `ownership` override) so `createSecureAgent` defaults stay immutable; per-logical-session serialization with branch-leaf carry-forward; process-wide active-session semaphore plus per-binding/per-process pending caps and a route cap; bounded in-process event dedup; final replies only from the current run's `succeeded` text with redacted bounded failure/cancel/suspend notices; `/help`/`/status`/`/new`/`/agent`/`/cancel` bypass the queue (no model call for unknown slash/config input); `/cancel` aborts the active run and drops queued turns. Deliberately deferred to Task 3+: durable bindings/pairing/journal and operation CAS claim (`checkpoints`/`leases` are therefore not yet factory options), platform adapters (Tasks 5/7) and approval controls (Task 6) — `deliver` is the host seam the adapters will feed. In-process state only: `/new` generations, seen-event dedup and suspension status reset on process restart. No core/shared-service change was required. Budgets: `@arnilo/prism-core` export ceiling rebaselined 1473 → 1498 (+25 channel exports, all on the new subpath, none in the root barrel) with the reason recorded in `scripts/budgets.json`; root tarball unchanged in budget terms after the docs page (rebaseline from Task 1 still passes).
    - Boundary change (2026-09-16, user-directed): this task's artifacts (`{index,types,limits,runtime}.ts`, `__tests__/runtime.test.ts`) were built under `packages/prism-core/src/integrations/channels/` and move to `packages/prism-channels/src/` in Task 4, together with the prism-core subpath export. Test counts, frozen symbol names, options and documented semantics are unchanged.
...
- [x] **Task 3 — Durable bindings, pairing, intake and reply journal**
  - Acceptance Criteria:
    ...
    - Task 2 hand-off: the runtime keeps bindings, `/new` generations, seen events and suspension status in memory; this task adds the `CheckpointStore`/`LeaseStore` options to `createMessagingRuntime` and replaces the in-process dedup/claim with the durable CAS claim before `session.run`.
  ...
  - Approach:
    ...
    - Boundary change (2026-09-16, user-directed): `{state,pairing,delivery}.ts` and their three suites were built under `packages/prism-core/src/integrations/channels/` and move to `packages/prism-channels/src/` in Task 4. The Task 1 primitive probe stays in prism-core (it examines core primitives, not channels). Journal namespaces, record schemas, limits and recovery semantics are unchanged.
...
- [x] **Task 4 — Extract the channels implementation into `@arnilo/prism-channels`** — completed 2026-09-16
  - Acceptance Criteria:
    - Functional: the code and tests delivered in Tasks 2–3 load and pass from the new package `@arnilo/prism-channels` (`packages/prism-channels`); `@arnilo/prism-core/integrations/channels` no longer exists in source, package exports, package map or current-contract docs; every symbol name frozen by Task 1 is unchanged — only the specifier moves.
    - Performance: the new package's build, packed size and cold-import budget are measured, not assumed; the extraction adds no runtime work (no I/O at import or factory construction still holds) and the packed package starts with only its declared peers installed.
    - Code Quality: no import escapes the package by relative path, and only `@arnilo/prism` (public entry) plus Node builtins are runtime imports; `@arnilo/prism-core` stays a dev-only dependency for the SQLite/PostgreSQL test legs; no channel runtime, journal or limits code is left duplicated in prism-core; package conventions match an existing family package (`sideEffects: false`, `files: ["dist", …]`, explicit test file lists instead of shell globs).
    - Security: the move preserves the reviewed boundaries (deny by default, ownership-embedded journal keys, staged replies, fail-closed leases, reauthorization before work and send); packaging adds no privileged entry point, listener, credential discovery or subprocess; Signal's Node-only code stays behind its own subpath.
  - Approach:
    - Documentation Reviewed: Task 2/3 outcomes and file lists; `packages/memory/{package.json,tsconfig.json}` and `packages/prism-core/package.json` as the family-package templates; `scripts/{release.mjs,package-truth.mjs,phase54-package-map.mjs,budgets.json,e2e-coverage.json,e2e-coverage.test.mjs,e2e-full-surface.test.mjs,coverage-thresholds.json,coverage-summary.json,live-matrix.json,import-hygiene.*}`; `docs/release-and-install.md`, `docs/peer-dependencies.md`, `docs/core.md`, `docs/index.md`.
    - Options Considered: keep the module in prism-core (current state; couples platform adapters to the core release, export budget and peer set, and grows a surface unrelated to the runtime — rejected by user direction 2026-09-16); one package per platform (Telegram and Signal separately) — premature while they share contracts, the journal and the runtime, and it would need a third package for the shared code (rejected for now); one `@arnilo/prism-channels` package with `/telegram` and `/signal` subpaths (chosen).
    - Chosen Approach: create `packages/prism-channels` with root, `/telegram` and `/signal` (the latter two added by Tasks 5/7); move `{index,types,limits,state,delivery,pairing,runtime}.ts` and the four channel suites, changing only import specifiers; move the Task 1 probe to `packages/prism-core/src/__tests__/messaging-primitives.test.ts` with an explicit entry in the prism-core test script (it tests core primitives, not channels); delete the prism-core module, its subpath export and its package-map/truth/e2e annotations; add the new package to workspaces, truth, package map, budgets, coverage and live-matrix inventories. `scripts/release.mjs` discovers packages by directory, so no release list edit is expected — verify it. Version the package with the family for 0.8.0; publication still needs operator authorization.
    - API Notes and Examples: `import { createMessagingRuntime, createChannelStateStore, createChannelDeliveryJournal, createChannelPairingStore } from "@arnilo/prism-channels"` replaces the prism-core specifier; host composition (`authorize`/`resolveAgent`/`deliver`/`resolveBinding`/`redactor`/`checkpoints`/`leases`/`limits`) and all documented semantics are unchanged.
    - Files to Create/Edit: `packages/prism-channels/{package.json,tsconfig.json,README.md,CHANGELOG.md}`; `packages/prism-channels/src/{index,types,limits,state,delivery,pairing,runtime}.ts` (moved); `packages/prism-channels/src/__tests__/{runtime,state,recovery,postgres.integration}.test.ts` (moved); `packages/prism-core/src/integrations/channels/**` (removed, except the probe which relocates); `packages/prism-core/{package.json,README.md}`; root `package.json`; `scripts/{package-truth.mjs,package-truth.json,phase54-package-map.mjs,budgets.json,e2e-coverage.json,e2e-coverage.test.mjs,e2e-full-surface.test.mjs,coverage-thresholds.json,coverage-summary.json,live-matrix.json}`; `docs/{core,index,messaging-channels,messaging-channel-operations,release-and-install,peer-dependencies}.md`; regenerated `docs/_evidence/phase54-package-map.md`; `README.md`; `plans/README.md`; `graft build`.
    - References: Task 2/3 outcomes, [P1–P8]; existing family-package manifests and the packaging/truth gates listed above.
  - Test Cases to Write: moved suites stay green (`node --test "packages/prism-channels/dist/__tests__/*.test.js"` plus the explicit PostgreSQL leg with `PRISM_TEST_POSTGRES_URL`); the Task 1 probe still runs from prism-core; packed smoke import of `.` with no optional peers and `PRISM_TEST_POSTGRES_URL` unset; `npm run pack:dry-run` for the new package packs only `dist` + README/LICENSE/CHANGELOG and contains no `@arnilo/prism-core` import; prism-core's packed surface no longer contains channel code or the `/integrations/channels` subpath; export-truth and package-map inventories list the moved symbols once, in the new package; behavior assertions (dedup, restart, reconcile, ownership, tool scope) unchanged.
  - Documentation/Wiki Assessment:
    - Public API or behavior impacted: yes — a new published package and the removal of a prism-core subpath.
    - Docs pages to create/edit: thin `packages/prism-channels/README.md` and its `CHANGELOG.md` stub; `docs/messaging-channels.md`, `docs/messaging-channel-operations.md` (specifier and install lines), `docs/core.md` (row removed), `docs/release-and-install.md`, `docs/peer-dependencies.md`; `docs/history/079-messaging-primitive-review.md` gets a superseded-boundary note only.
    - `docs/index.md` update: yes — family list plus the messaging entry's package name.
    - Documentation structure reference: `.agents/skills/create-plan/references/prism-wiki.md`.
  - Task 4 Outcome (2026-09-16): moved the runtime, durable journal and four channel suites into `packages/prism-channels`; relocated the seven core-only primitive probes to `packages/prism-core/src/__tests__/messaging-primitives.test.ts`; removed the prism-core subpath and restored its export baseline. New package has only `@arnilo/prism` at runtime (peer); prism-core is test-only. Package truth, package map, budgets, coverage, packed-install journey, docs, release compatibility baseline and workspace inventories now list the surface once. Verified `npm test` (all five stages), `npm run test:postgres` (including PostgreSQL channel conformance), package packing and a packed peer-only import. Measured package tarball: 22,470 packed bytes / 102,713 unpacked bytes / 18 files; 25 cold-process imports had a 61.88 ms median (56.26–64.44 ms), recorded as the Task 9 comparison baseline. No import or factory I/O added. `deadExports: null` is now correctly treated as zero candidates by the existing verifier.

- [x] **Task 5 — Telegram adapter: polling and optional webhook ingress** — completed 2026-09-16
  - Acceptance Criteria:
  ...
  - Approach:
    ...
    - Files to Create/Edit: `packages/prism-channels/src/telegram.ts`; shared `output.ts` if needed by both adapters; `packages/prism-channels/src/__tests__/telegram.test.ts`; `packages/prism-channels/package.json` (subpath); `examples/telegram-agent.ts`; `docs/telegram-channel.md`.
  ...
  - Documentation/Wiki Assessment:
    ...
    - Documentation structure reference: `.agents/skills/create-plan/references/prism-wiki.md`.
  - Task 5 Outcome (2026-09-16): shipped the isolated `@arnilo/prism-channels/telegram` subpath with native-fetch `createTelegramAdapter` and mountable `createTelegramWebhookHandler`; no bot framework, runtime dependency, import/factory I/O, listener, environment lookup or webhook administration was added. Polling explicitly verifies `getMe`, rejects a configured webhook, holds a durable service-owned receiver lease, waits for runtime admission before persisting the Telegram offset, handles post-gap IDs without assuming sequential numbering, and fences webhook/poll mode on the same connection. The handler uses constant-time secret comparison, bounded JSON, lease-fenced admission and `503` for capacity/storage/stopped paths. Only private non-bot text becomes a runtime event; callbacks are acknowledged but inert pending Task 6. Plain-text sends are fixed to the bound chat, Unicode-safe chunked at 3,500 UTF-16 units, honor `retry_after`, forbid redirects and make network/partial sends `delivery_unknown` rather than automatically retryable. Added 11 network-free adapter tests (including runtime routing/final reply, cursor replay, webhook, lease, rate limit, chunking and URL redaction), package/e2e/export/compat coverage, `examples/telegram-agent.ts`, and `docs/telegram-channel.md`; `ChannelReceive` lets adapters retain acknowledgment on capacity/journal failure. Export ceiling measured 63 → 70. Verified `npm run typecheck`, channels package tests (39 passing, PostgreSQL leg skips without its explicit URL), `npm run test:coverage`, and package/truth/map/budget/packaging gates. Official Bot API facts rechecked: webhook/polling exclusivity, offset confirmation, secret header, 4,096-text/64-byte callback limits and `retry_after`.

- [x] **Task 6 — Approval, status and cancellation integration**
  - Acceptance Criteria:
  ...
  - Approach:
    ...
    - Files to Create/Edit: `packages/prism-channels/src/approvals.ts`; runtime and adapter files; focused runtime/Telegram approval tests; `docs/messaging-channels.md`, `docs/telegram-channel.md`, `docs/messaging-channel-operations.md`, `docs/signal-channel.md` when Signal ships. Generic resume or core changes, if proven necessary, are `@arnilo/prism` work scoped separately (see Further Actions), never a prism-channels change to core internals.
  ...
  - Documentation/Wiki Assessment:
    ...
    - Documentation structure reference: `.agents/skills/create-plan/references/prism-wiki.md`.
  - Task 6 Outcome (2026-09-16): shipped `approvals.ts` and the shared runtime control path. A generated opaque token hashes into the durable control journal and binds one `allow_once`/`reject_once` decision to principal, grant revision, conversation/thread, agent alias/definition revision, session, run, pending approval, expected checkpoint version and five-minute expiry. Tokens are CAS-consumed before `AgentRunLifecycle.resumeStream`; callback and command-shaped controls never reach model input. Resumption reauthorizes and re-resolves the agent, records a fresh control reply before delivery, sends only current terminal succeeded text, and re-suspends for the next decision. `/status` restores a suspended binding after restart; `/cancel` terminally denies a suspended run through core and aborts an in-flight resume without claiming dispatched work rolled back. Telegram now renders the bounded controls as inline callbacks and acknowledges them before durable admission. Added runtime coverage for restart, forwarded/replayed/revoked controls, denial and cancellation during resume, plus Telegram callback/keyboard coverage. Verified with the package test suite (42 passing; PostgreSQL skipped without its configured URL).

- [x] **Task 7 — Experimental Signal adapter and policy/compatibility gate** — completed 2026-09-16
  - Acceptance Criteria:
  ...
  - Approach:
    ...
    - Files to Create/Edit: `packages/prism-channels/src/signal.ts`; `packages/prism-channels/src/__tests__/signal.test.ts` (pinned sanitized envelope builders live inline in the suite; no separate fixtures directory — the shape is small enough to inline without a fixture framework); `packages/prism-channels/package.json` (Node-only `/signal` subpath); `examples/signal-agent.ts`; `docs/signal-channel.md`; `docs/messaging-channel-operations.md`; `docs/messaging-channels.md`; `docs/index.md`; `docs/server.md`; `docs/release-and-install.md`; `packages/prism-channels/README.md`; `packages/prism-channels/CHANGELOG.md`; `examples/README.md`; `scripts/{package-truth.mjs,phase54-package-map.mjs,budgets.json,e2e-coverage.json,e2e-coverage.test.mjs,fixtures/e2e-full-surface-journey.mjs,compat-baseline/arnilo__prism-channels.txt}` plus regenerated truth/map evidence.
  - Task 7 Outcome (2026-09-16): shipped the isolated Node-only `@arnilo/prism-channels/signal` subpath with `createSignalAdapter` (+ `SIGNAL_CLI_VERSION`, `SignalPolicyGate`, `SignalAdapterOptions`, `SignalAdapterHealth`, `SignalAdapter`). Import and construction are inert — no spawn, download, registration, link, TCP/HTTP bridge, environment or credential lookup; only `start()` connects the host-selected private Unix socket. `start()` acquires a service-owned `prism.channels.v1.signal.receiver` lease, probes the checkpoint writer (startup fails before subscription when storage is unavailable), then subscribes via `subscribeReceive` only; renews the lease, pauses/unsubscribes on writer/capacity/frame/account/lease trouble, and reconnects with bounded exponential backoff only after an established bridge loss (a disconnect before subscription fails startup instead of looping). Only manual `params.result.envelope` notifications naming the selected account and carrying a direct text `dataMessage` from a valid Signal UUID become runtime events; groups, attachments, receipts, sync echoes, reactions, stories, edits, foreign accounts, malformed wrappers and unrelated RPC are filtered before `admit`. UUID (not phone/display name) is the external actor/conversation id; `health()` reports independent payload-free bridge/subscription/account states. `send()` calls only the `send` RPC to the runtime-fixed UUID, chunks plain text at 2,000 UTF-16 units, renders only server-shaped approval controls as `/approve <token>` / `/deny <token>` command text, maps known daemon errors (`signal_rate_limited` / `signal_captcha_required` / `signal_relink_required` / `signal_identity_changed` / `signal_rpc_error`), refuses changed identities until operator re-verification, and throws ambiguous-delivery on timeout/socket loss (journal records `delivery_unknown`, never auto-resent). Construction rejects a missing/malformed operator `policy` attestation or a non-pinned `signalCliVersion`. Added 11 network-free adapter tests (inert construction, runtime routing/final reply, filtering, account mismatch, capacity pause/reconnect, writer-unavailable startup, pre/post-subscription disconnect, oversized-frame drop/reconnect, kill-between-receipt-and-commit, error mapping/identity/ambiguity, approval rendering) plus the packed-install smoke probe; `examples/signal-agent.ts` and `docs/signal-channel.md` document the externally supervised daemon, private-socket/`0700` posture, GPLv3/terms policy gate, `/signal` package-map/truth/e2e/export/compat coverage (104 e2e surfaces, compat baseline 70 → 72 declared exports, src-export budget 74 → 80). Verified `npm run typecheck`, channels package suite (53 passing), full `npm run test:coverage` (channels lines 89.09 ≥ 86.13 threshold; both coverage gate suites pass), `npm run lint`, `npm run format:check`, e2e coverage/full-surface, budget and packed tarball (24 files, no `__tests__`/maps in pack).
...
- [x] **Task 8 — Host examples, deployment and fault-injection verification** — completed 2026-09-16
  - Acceptance Criteria:
  ...
  - Approach:
    ...
    - API Notes and Examples: `createSqlitePersistence({ filename })` supplies `store`/`runLedger` on `AgentConfig`, and `checkpoints`/`leases` to the channel runtime; host closes persistence after drain. `npm run typecheck`, `npm run test -w @arnilo/prism-channels`, `npm run test:postgres` follow existing build/config gates.
    - Files to Create/Edit: `examples/messaging-agent.ts`, `examples/telegram-agent.ts`, `examples/signal-agent.ts`, `examples/README.md`; `packages/prism-channels/src/__tests__/{end-to-end,telegram-live,signal-live}.test.ts`; `scripts/fixtures/messaging-restart-worker.mjs`; `scripts/live-matrix.json`, `scripts/live.env.example`; `docs/messaging-channel-operations.md`, `docs/live-testing.md`.
  ...
  - Documentation/Wiki Assessment:
    ...
    - Documentation structure reference: `.agents/skills/create-plan/references/prism-wiki.md`.
  - Task 8 Outcome (2026-09-16): shipped a network-free sqlite host demo (`examples/messaging-agent.ts`), Telegram webhook mount helper (`mountTelegramWebhook`), SIGKILL restart worker, and opt-in live probes that skip without credentials and never create contacts/accounts. End-to-end suite covers sqlite persist/reopen/duplicate, two-host lease fence, queue flood, revoke-before-dispatch, duplicate-admission p95 (CI bound 500 ms; operator target remains <100 ms on a reference SSD), and SIGKILL → authorized `reconcile` (`execution_unknown`) without a second run. `reconcile` now reuses the stored checkpoint fencing token so a crashed lease holder can be filed as a dead-letter on sqlite/postgres (memory-store recovery tests never hit this). PostgreSQL journal conformance stays the existing `test:postgres` leg — no new SQL adapter. Live matrix: `channels/telegram-live` (`PRISM_LIVE_TELEGRAM` + `TELEGRAM_BOT_TOKEN`, optional chat id) and `channels/signal-live` (socket/account/terms, optional recipient UUID). 72-hour soak is an operator recipe in `docs/messaging-channel-operations.md`, not a CI runner. Graceful shutdown is `runtime.stop()` (already covered); OS SIGTERM process injection was skipped as flaky against sqlite. Package suite 59 passing (6 new host tests); live tests skip by default. Verified `npm run test -w @arnilo/prism-channels`, examples typecheck, `lint`/`format:check`, live-matrix `--check`, e2e-coverage/full-surface, live-doc-check.

- [x] **Task 9 — Packaging, current-contract documentation and final verification** — completed 2026-09-16
  - Acceptance Criteria:
  ...
  - Documentation/Wiki Assessment:
    ...
    - Documentation structure reference: `.agents/skills/create-plan/references/prism-wiki.md`.
  - Task 9 Outcome (2026-09-16): current-contract docs for messaging/Telegram/Signal/operations now carry prism-wiki headings and sit on `apiPages`; options-index lists host option surfaces; peer-dependencies notes channels has no third-party peers; core.md points channels out of prism-core; index current-line is 11 packages. Packed isolation is a root packaging guard (root/telegram never load `./signal` or `node:net`; tarball has no tests/fixtures; only `@arnilo/prism` peer). install-smoke imports `./telegram` and `./signal`. Did not add four package-local packaging/docs/install-smoke/public-export-contract files — root gates already own those. Unused Signal `open`/`startReconnect` params removed so biome lint is clean. Verified `npm test` (all 5 stages), `lint`, `format:check`, budget gates, examples typecheck, `pack:dry-run` (24 files, 40.5 kB). No version bump or publish.

## 6. Sequencing, estimates and acceptance gates

Sequence: Task 1 -> Task 2 -> Task 3 -> **Task 4 (extraction to `@arnilo/prism-channels`)** -> Task 5 -> Task 6 -> Task 7 -> Task 8 -> Task 9. Task 4 comes first because every later adapter and control is written against the new package; Tasks 2–3 stay valid because their names, semantics and tests move unchanged. Signal policy/account feasibility can be reviewed during Task 1; Telegram release need not wait for Signal approval. Task 6 common controls can be tested with fake adapters before Signal implementation.
...
| Package extraction to `@arnilo/prism-channels` (move, plumbing, budgets, docs) | 2–3 |
...
- Task 1 (2026-09-15) resolved the default execution path to direct `AgentSession` plus `AgentRunLifecycle.resumeStream`; `ConversationService` composition with secure agents is refused rather than fixed in this plan, and `AgentRunLifecycle.resume`'s dropped `signal` is documented with the `resumeStream` alternative (optional generic fix if Task 2 selects it).
- Root tarball diet budget rebaselined on 2026-09-15 (measured 1247731/4137527/489) because the mandated history review document and unabsorbed post-0.7.0-cut growth left no packed headroom; the measured split is recorded in `scripts/budgets.json` and Task 8 re-verifies it.
- Signal policy gate: no acceptable-use decision exists yet. The review recorded the license analysis (JSON-RPC socket client does not trigger GPLv3 obligations) and the Signal terms constraints; a human operator decision remains a Task 7 release gate.
- Task 2 (2026-09-15) ships in-process-only runtime state: bindings, `/new` generations, seen-event dedup and suspension status do not survive a restart until Task 3 adds the durable journal; the runtime options therefore omit `checkpoints`/`leases` and the durable admission CAS claim instead of accepting unused options.
- Task 2 delivers through a host `deliver` callback instead of taking an adapter dependency, so the runtime stays transport-free and testable; Telegram (Task 5) wires it as `deliver: (reply) => adapter.send(reply)`, and Signal (Task 7) will use the same host seam. Durable reply state (Task 3) wraps it rather than replacing it.
- Approval controls are not part of Task 2: a suspended run sets `route.suspended` and ordinary turns are denied with `awaiting_decision` until Task 6 adds the durable approval control path (`/new` clears it locally as an escape hatch).
- **Package boundary changed on 2026-09-16 by user direction.** Channels move from `@arnilo/prism-core/integrations/channels` to the new `@arnilo/prism-channels` package (Task 4), because more transports are expected and each brings its own client, peers and release risk. Consequences accepted: one more first-party package to version, publish and verify; Tasks 2–3 artifacts must be moved and their prism-core subpath, package-map, truth, coverage and budget entries removed (prism-core's export ceiling returns to its pre-079 value); Task 1's frozen *subpath* decisions are superseded while its frozen *symbol names* still hold; the primitive probe stays in prism-core because it tests core primitives. Cost and risk are the migration task itself plus the new package's packaging gates; nothing in the runtime, journal or authorization semantics changes.
- Task 3 (2026-09-16) keeps the journal deliberately narrow: no exactly-once claim, no automatic replay on startup, no channel SQL schema (generic `CheckpointStore`/`LeaseStore` only), and no payload archive — the only payload kept is the bounded staged reply text, pruned after `retentionDays`. An `accepted` operation whose process died is a dead-letter (no inbound text is retained, so a retry is a user resend), and a deferred (lease-held-elsewhere) event drops its unclaimed record so a redelivery can still be admitted.
- Task 3 leases are per binding and taken lazily at claim time (released when the route idles), so they fence concurrent double-driving of one binding, not sequential handover: two workers can alternate turns on the same binding when `leases` is configured. A stronger "one receiver per connection" lease belongs with the adapters (Tasks 5/7) where the receive loop owns that lifecycle.
- Task 3 did not measure the durable-admission p95 target (<100 ms on a reference Linux SSD excluding network/model): the SQLite/PostgreSQL legs prove correctness, not latency, and the local runs are test-sized. Task 8 times duplicate durable admission in the sqlite host suite with a CI bound of 500 ms so slow runners do not flake; the operator target remains <100 ms. Task 9 re-verifies the recorded numbers.
- Task 8 does not inject OS SIGTERM into the sqlite worker (flaky against an in-process journal write); abrupt crash is SIGKILL + authorized `reconcile`, and graceful shutdown is `runtime.stop()`. A 72-hour soak is documented as an operator recipe, not a CI runner.
- Task 9 did not add package-local `docs`/`packaging`/`install-smoke`/`public-export-contract` suites: root `src/__tests__/packaging.test.ts`, `install-smoke.test.ts`, `docs.test.ts` and `scripts/compat-baseline/` already own those gates. Isolation is one packaging assertion plus install-smoke subpath imports. No 0.8.0 version bump or publish from this plan.
- Task 3 `reconcile` cannot replay model/tool work and cannot recover an `accepted` operation: without retained inbound text the only honest outcomes are `abandoned` (nothing ran) and `execution_unknown` (may have run). Operator-requested replay with a resupplied prompt is a possible future control, not implemented here.

## Further Actions

Moved to [080](080-Messaging-Channel-Followons-And-0-8-0-Cut.md): group/topic ownership; Telegram draft streaming; bounded attachments; voice via existing modality APIs; opt-in notify; ERP outbox composition; SIGTERM + 72h soak; leftover Task 1 primitive gaps; 0.8.0 cut.

Discovered during Task 1 (2026-09-15):

- **Nested prism-core suites are outside the default test chain.** Every package `test` script relies on shell expansion of `dist/**/...`, so `**` behaves as `*` and only one-level `__tests__` folders run under `npm test` (`prism-core` currently runs just `runtime/__tests__/realtime.test.js`). Nested suites like `runtime/server/__tests__/conversations.test.ts`, `sessions/sqlite/__tests__`, and this review's `integrations/channels/__tests__/primitives.test.ts` only run when named explicitly (or via Node's quoted glob). Fixing it means switching the package test scripts to resolved file lists or quoted patterns — an isolated, repo-wide test-topology change with its own risk of surfacing previously unrun suites; out of scope for plan 079, decide separately. Task 9 must keep the explicit probe command in its evidence.
- **`AgentRunLifecycle.resume()` drops its `signal`.** `AgentRunResumeOptions` has no `signal` field while `AgentRunResumeStreamOptions` does, so only the stream path cancels mid-run resumes. The channel MVP uses `resumeStream`, so this stays an optional generic core fix (add the field and thread it through `resumeAgentRun`) rather than a plan 079 requirement.
- **Checkpoint stores expose foreign-scope existence.** `MemoryCheckpointStore` keys on `namespace + key` only, so a cross-tenant probe for another owner's key throws `Checkpoint ownership mismatch` instead of missing; DB stores behave the same by inspection. Channel keys embedding tenant/user avoid collisions, but if a shared checkpoint store ever serves multiple channel owners, generic stores should return not-found for non-matching scopes rather than disambiguating by throwing.

Discovered during Task 2 (2026-09-15):

- **Suspended runs block their binding with no in-plan resolution until Task 6.** Task 2 sets `route.suspended` and denies further ordinary turns (`awaiting_decision`) so an unresolved tool decision cannot be bypassed; `/new` resets it locally. Task 3 persists the blocked state so a restart cannot silently unblock a pending decision; Task 6 must still add the durable approval path that clears it.
- **In-process seen-event dedup is bounded, not authoritative.** Duplicate webhook/poll retries within one process are acknowledged and skipped, but the tombstones are capped and lost on restart; Task 3's durable CAS claim is the real deduplication guarantee.
- **`maxRoutes` is a Task 2 addition to the Task 1 limits table.** Routes (one per connection + conversation + thread + actor + alias) hold the in-memory binding state, so the runtime caps them at 1,000 (hard 10,000) and denies with `capacity`; add the row to the review table if the durable journal keeps the same map shape.

Discovered during Task 3 (2026-09-16):

- **`listUnresolved` is the version source for reconciliation.** A host cannot reload an operation version from the runtime's public surface (it can read it from its own checkpoint store). If reconciliation becomes common, consider returning the opened version directly from `admit`/`deliver` rather than requiring a list call.
- **`CheckpointStore.listCheckpoints` is OFFSET-paginated and not a snapshot.** `prune` therefore deletes only what its bounded page returned and never treats a page walk as consistent; repeated calls make progress. A high-churn host should schedule it off the message path.
- **Reply staging costs one extra read per send.** `sendReply` loads the reply record before staging so a settled reply is never resent automatically; that read is what makes `delivery_unknown` truthful, and it doubles as the duplicate-send guard.
- **Memory-store eviction interacts with open work.** `createMemoryCheckpointStore` evicts least-recently-saved records past 10 000, so an intensive local test run can drop unresolved operations. Durable deployments must use SQLite/PostgreSQL stores; documented in the operations guide.
