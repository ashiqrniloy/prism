...
- [x] Task 6 — Migration/docs finalization, 0.2.2 bump, and fail-loud exit gate
  ...
  - Status: complete (2026-08-13) at the 0.2.2 working-tree state. Evidence and deltas:
    ...
    - Post-release CI remediation (2026-08-14): the release workflow's postgres-integration job failed phase12's frozen point-op ceiling — append contention cold p95 320.99ms vs 50ms — while the append SQL is byte-identical to 0.2.1 (git diff empty for the probe, worker, and event-source). Root cause: migration 008's `ALTER TABLE prism_sessions` is the first post-001 DDL taking ACCESS EXCLUSIVE on the table every append upserts; on the 2-vCPU runner the 16 serialized straggler migration batches (advisory-lock serialized) blocked concurrent appends (~20ms per window). Fix: the contention probe now warms the schema once before the 16 appenders (new worker mode `warm` — applies migrations and seeds the probe session/stream rows; re-opens only verify checksums), so the measured point op is steady-state 16-way hot-row contention. Local warm evidence: contention p95 8.19ms vs 50ms ceiling (ceiling unchanged); reconnect p95 199ms vs 2000ms. Deviation dev-002 recorded in `scripts/phase12-freeze-manifest.json` `featureFreeze.deviations`; evidence refreshed in `scripts/phase12-restart-recovery.json`; phase22-baseline testPostgres note updated. CI re-run pending green.
