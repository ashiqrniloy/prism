...
- **Any task that moves, renames, or removes a public symbol/export — including package splits and re-export moves — must plan the compat-baseline regeneration explicitly.** Name `node scripts/release.mjs gate --update-baseline` (and the `scripts/compat-baseline/` files it rewrites) in that task's `Files to Create/Edit`, require the post-regeneration `release:gate` diff in the task note, and state which removals are this plan's versus inherited. Plan 083 split `@arnilo/prism-office` into `@arnilo/prism-work` without regenerating, which left `release:gate` red for every later plan; plan 084 Task 8 absorbed it.
