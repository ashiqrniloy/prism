import { createAgent, createAgentSession, createExtensionKernel, } from "@arnilo/prism";
import { createNeuralWattProviderPackage, defineNeuralWattModel, neuralWattEventsWithTelemetry, } from "@arnilo/prism-provider-neuralwatt";
const requestBodies = [];
...
async function telemetry() {
    let energy;
    let cost;
    for await (const event of neuralWattEventsWithTelemetry(sse([': energy {"energy_kwh":0.00042,"duration_seconds":0.3}', ': cost {"request_cost_usd":0.0009,"cache_savings_usd":0.0012}']))) {
...
export async function demo() {
    ...
    return {
        ...
        enableThinking: requestBodies[0]?.chat_template_kwargs?.enable_thinking,
        preserveThinking: requestBodies[0]?.chat_template_kwargs?.preserve_thinking,
        clearThinking: requestBodies[0]?.chat_template_kwargs?.clear_thinking,
