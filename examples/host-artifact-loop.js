...
import { createAgent, createMockProvider, createProviderResolver, createSecretRedactor, createSkillRegistry, createToolRegistry, providerDone, providerTextDelta, providerToolCall, } from "@arnilo/prism";
...
const validator = (value) => {
    ...
    if (!note.body)
        errors.push({ path: "body", message: "missing body" });
    return errors.length === 0 ? { ok: true } : { ok: false, errors };
};
const repairer = (_value, failure) => ({
    role: "user",
    content: [
        {
            type: "text",
...
const firstPartyEcho = {
    ...
    execute(args, ctx) {
        return { toolCallId: ctx.toolCallId, name: "first-party/echo", value: args.text };
    },
};
const thirdPartyFetchSchema = {
...
async function setupTempPromptFiles() {
...
async function runToolDispatchDemo(tools, skills, layers) {
...
async function runArtifactLoopDemo(tools, skills, layers) {
...
export async function main() {
    ...
    if (!result.redacted)
        throw new Error("expected fake secret to be redacted");
    if (!result.finishedOk)
        throw new Error("expected artifact_finished with ok === true");
    const expected = [
        "artifact_validation_started",
        "artifact_validation_finished",
        "artifact_revision_started",
