...
import { createWorkflowCheckpoints, createWorkflowCoordinator, createWorkflowSchedules, defineWorkflow, functionNode, replayWorkflow, workflowNode, } from "@arnilo/prism-workflows";
const ownership = { tenantId: "demo", userId: "operator" };
...
while (coordinator.activeRuns > 0)
    await new Promise((resolve) => setTimeout(resolve, 1));
const fired = await schedules.get("report-once");
if (!fired?.lastRunId)
    throw new Error("schedule did not enqueue a run");
