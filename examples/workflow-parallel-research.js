import { createAgent, createAgentSession, createMockProvider, createSecretRedactor, providerDone, providerTextDelta, providerUsage, } from "@arnilo/prism";
import { agentNode, createMemoryWorkflowCheckpoints, defineWorkflow, fanOutNode, functionNode, joinNode, runWorkflow, } from "@arnilo/prism-workflows";
...
const fanOut = fanOutNode({
    items: (ctx) => ctx.workflowInput.topics,
    map: (item) => String(item),
    maxFanOut: 3,
});
const topics = joinNode({ from: "fanOut" });
const research = (index) => functionNode({
    execute: async (ctx) => {
        await new Promise((resolve) => setTimeout(resolve, 10));
        const list = ctx.upstream.topics;
...
export async function demo() {
    ...
    const events = [];
    const result = await runWorkflow(workflow, { topics: ["hooks", "signals", "suspense"] }, {
    ...
    const findings = result.outputs.combine?.findings;
    return {
