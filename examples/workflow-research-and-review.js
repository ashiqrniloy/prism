import { createAgent, createAgentSession, createMockProvider, createSecretRedactor, createToolRegistry, providerDone, providerTextDelta, providerUsage, } from "@arnilo/prism";
import { agentNode, createMemoryWorkflowCheckpoints, defineWorkflow, runWorkflow } from "@arnilo/prism-workflows";
...
function makeAgent(_name, system) {
    return createAgent({
...
export async function demo() {
    ...
    const events = [];
    const result = await runWorkflow(workflow, { topic: "React hooks" }, {
        agentFactory: (name) => createAgentSession({
