import { assembleProviderInput, createLoadedSkillSet, createLoadSkillTool, createSkillRegistry, createToolRegistry, dispatchToolCall, toolCallContent, } from "@arnilo/prism";
function catalogSkills(count) {
...
export async function demo() {
    ...
    const catalogText = catalogRequest.messages
        ...
        .join("\n");
    const catalogOnly = skills.every((skill) => catalogText.includes(`Skill ${skill.name}: ${skill.description}`) && !catalogText.includes((skill.instructions ?? "").slice(0, 40)));
