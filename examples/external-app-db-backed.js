import { createAgent, createAgentSession, createMockProvider, createProviderResolver, createSecretRedactor, createSkillRegistry, createToolRegistry, providerDone, providerTextDelta, providerToolCall, providerUsage, SessionAppendConflictError, } from "@arnilo/prism";
...
const saveNoteTool = {
    ...
    execute(args, ctx) {
        return { toolCallId: ctx.toolCallId, name: "notes/save", value: args.text };
    },
};
...
function createDbBackedReferenceStore() {
    ...
    const toolCalls = [];
    const usage = [];
    const bySession = (sid) => [...entries.values()].filter((e) => e.sessionId === sid).sort((a, b) => a.timestamp.localeCompare(b.timestamp));
    const readBranchPath = async (query) => {
        ...
        if (!leafId) {
            ...
            const childTargets = new Set(scoped.map((e) => e.parentId).filter((p) => Boolean(p)));
    ...
    return {
        ...
        async queryEntries(query) {
            ...
            if (query.sessionId)
                rows = rows.filter((e) => e.sessionId === query.sessionId);
            if (query.runId)
                rows = rows.filter((e) => e.runId === query.runId);
        ...
        async queryRuns(query) {
            ...
            let rows = runs;
            if (query.sessionId)
                rows = rows.filter((r) => r.sessionId === query.sessionId);
            return { items: rows };
        },
        ...
        async queryToolCalls(query) {
            let rows = toolCalls;
            if (query.sessionId)
                rows = rows.filter((c) => c.sessionId === query.sessionId);
            if (query.runId)
                rows = rows.filter((c) => c.runId === query.runId);
            if (query.name)
                rows = rows.filter((c) => c.name === query.name);
            return { items: rows };
        },
        async queryUsage(query) {
            let rows = usage;
            if (query.sessionId)
                rows = rows.filter((u) => u.sessionId === query.sessionId);
            if (query.runId)
                rows = rows.filter((u) => u.runId === query.runId);
            return { items: rows };
        },
...
// --- Demo driver -----------------------------------------------------------
async function collectEvents(subscription) {
    const out = [];
...
export async function demo() {
    ...
    return {
        ...
        resumedUsageRuns: resumedUsage.items.map((u) => u.runId).filter((r) => Boolean(r)),
...
export async function main() {
    ...
    if (!result.branchHandleCheckout)
        throw new Error("expected checkout to re-point the branch leaf");
    if (!result.forkDiverged)
        throw new Error("expected the forked branch to diverge from the shared leaf");
    if (!result.toolCallNames.includes("notes/save"))
        throw new Error("expected notes/save tool call persisted in ledger");
    if (result.resumedRunCount < 1)
        throw new Error("expected at least one run resumable from the ledger");
    if (result.resumedEventTypes.length === 0)
        throw new Error("expected run-1 events resumable from the ledger");
    if (result.resumedUsageRuns.length === 0)
        throw new Error("expected run-1 usage resumable from the ledger");
    if (!result.secretRedactedFromLedger)
        throw new Error("expected raw secrets redacted from the persisted ledger/entries");
    if (!result.credentialNeverLogged)
        throw new Error("expected caller credential value to never enter the event stream");
    if (!result.conformancePassed)
        throw new Error("expected DB-backed reference store to pass SessionStore conformance");
}
