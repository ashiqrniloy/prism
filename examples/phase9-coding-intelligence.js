...
import { createBoundGitRunner, createGitAwareRepositoryOperations, createGitHubForge, createLanguageIntelligence, createProcessSessions, } from "@arnilo/prism-coding-agent";
...
function proxyGet(proxyPort, target, hostHeader) {
    return new Promise((resolve, reject) => {
        const req = httpRequest({ host: "127.0.0.1", port: proxyPort, path: target, method: "GET", headers: { host: hostHeader }, setHost: false }, (res) => {
            let body = "";
...
async function main() {
    ...
    try {
        ...
        const git = async (...args) => {
            ...
            if (r.exitCode !== 0)
                throw new Error(r.stderr.toString("utf8") || `git ${args[0]} failed`);
        };
        await git("init");
        ...
        try {
            ...
            for (let i = 0; i < 50; i += 1) {
                ...
                saw += chunk.data;
                cursor = chunk.cursor;
                if (saw.includes("test run captured"))
                    break;
        ...
        const fetchImpl = ((input, init) => {
            ...
            if (url.host === "api.github.com")
                return fetch(`http://127.0.0.1:${fakePort}${url.pathname}${url.search}`, init);
