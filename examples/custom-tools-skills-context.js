import { createAgent, createAgentSession, createMockProvider, createToolRegistry, providerDone, providerTextDelta, providerToolCall, providerUsage, } from "@arnilo/prism";
    ...
    async function drain() {
        for await (const event of session.subscribe()) {
            ...
            if (event.type === "tool_execution_started")
                toolCallNames.push(event.call.name);
            if (event.type === "message_delta" && event.content.type === "text")
                textDeltas.push(event.content.text);
        }
    }
