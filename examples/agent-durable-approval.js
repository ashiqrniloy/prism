...
export async function demo() {
    ...
    const agent = createAgent({
        ...
        provider: {
            ...
            async *generate() {
                if (++turn === 1)
                    yield { type: "tool_call", call: toolCallContent("write-1", "write", { value: "approved" }) };
                else
    ...
    if (suspended.status !== "suspended" || writes !== 0)
        throw new Error("expected pre-tool suspension");
