import { createAgent, createMemorySessionStore, createMockProvider, providerDone, providerTextDelta, providerToolCall, toolCallContent, } from "@arnilo/prism";
import { buildObservationalMemoryProjection, createObservationalMemory, recallObservationalMemory, recallObservationalMemoryBranchPage, renderObservationalMemory, } from "@arnilo/prism-compaction-observational-memory";
const model = { provider: "mock", model: "demo" };
const workerModel = { provider: "mock", model: "memory" };
function memoryWorkerProvider(store, sessionId) {
    return {
        id: "memory",
        async *generate() {
            ...
            yield providerToolCall(toolCallContent("c1", "record_observation", {
...
export async function demo() {
    ...
    const recall = observationId ? recallObservationalMemory(entries, observationId) : { found: false };
