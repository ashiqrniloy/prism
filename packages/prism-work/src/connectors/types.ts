...
export interface IdempotencyStore {
   ...
   *
   * Adapters reject with the portable codes `ERR_PRISM_WORK_IDEMPOTENCY` (rejected
   * claim/transition) and `ERR_PRISM_WORK_IDEMPOTENCY_CONFLICT` (lost race or stale
   * claim token). The error class is adapter-specific — a memory store throws
   * `WorkToolError`, the PostgreSQL adapter throws `EnterprisePostgresError` — so
   * hosts match on {@link WorkToolError.code}, never on `instanceof`.
   */
...
export interface WorkFileArtifactStore {
  readonly bodies: ArtifactBodyStore;
  /** Host owns artifact naming and ownership; work tools supply verified download metadata. */
  createRef(input: {
...
export interface WorkToolsOptions {
  ...
  /** File-byte limit for tool-side artifact and filesystem transfers; match adapter limits when overridden. */
  ...
  /** Optional contained filesystem used for downloaded files and sandbox-source uploads. */
  readonly filesystem?: WorkSandboxFilesystem;
  /** Optional host-owned artifact body store for downloaded files and sandbox-source uploads. */
  readonly artifacts?: WorkFileArtifactStore;
