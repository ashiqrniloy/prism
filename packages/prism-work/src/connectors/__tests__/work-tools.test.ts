...
describe("createWorkTools", () => {
  ...
  it("keeps the connector tool catalog stable", () => {
    const runner = fakeRunner(() => ({ exitCode: 0, stdout: "[]", stderr: "" }));
    const names = createWorkTools({
      microsoft365: createMicrosoft365CliAdapter({
        binary: "/usr/bin/m365",
        identity,
        configDir: "/tmp/m365-catalog",
        allowedOps: DEFAULT_M365_OPS,
        runner,
      }),
      googleWorkspace: createGoogleWorkspaceCliAdapter({
  ...
  it("keeps work-tool sources below 800 lines", () => {
    for (const file of ["tools.ts", "tool-helpers.ts", "m365-tools.ts", "gws-tools.ts"]) {
      const lines = readFileSync(new URL(`../../../src/connectors/${file}`, import.meta.url), "utf8").split("\n").length;
...
describe("google-workspace argv templates", () => {
  it("builds verified gws command shapes and rejects auth/schema/anyone", () => {
    ...
    assert.deepEqual(
      buildGoogleWorkspaceArgv("docs.update", {
        documentId: "doc-1",
        replaceAllText: { containsText: "old", replaceText: "new" },
        insertText: { locationIndex: 1, text: "intro" },
      }),
      [
        "docs",
        "documents",
    ...
    assert.deepEqual(buildGoogleWorkspaceArgv("sheets.update", { spreadsheetId: "sheet-1", range: "Sheet1!A1", values: [["=1+1"]] }), [
      "sheets",
      "spreadsheets",
      "values",
      "update",
      "--params",
      '{"spreadsheetId":"sheet-1","range":"Sheet1!A1","valueInputOption":"RAW"}',
      "--json",
      '{"values":[["=1+1"]]}',
    ]);
    assert.deepEqual(
      buildGoogleWorkspaceArgv("slides.update", { presentationId: "slide-1", insertText: { objectId: "shape-1", text: "Hello" } }),
      [
        "slides",
        "presentations",
        "batchUpdate",
        ...
        "--json",
        '{"requests":[{"insertText":{"objectId":"shape-1","text":"Hello"}}]}',
      ],
    );
    assert.throws(() => buildGoogleWorkspaceArgv("docs.update", { documentId: "doc-1", requests: [] }), /not allowed/);
