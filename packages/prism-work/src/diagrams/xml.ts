...
export function assertNotVisio(input: string | Uint8Array): void {
  if (input instanceof Uint8Array) {
    if (input.length >= 8) {
      ...
      if (isOle) {
        throw new DiagramsFormatError("Visio binary format (.vsd) is excluded and unsupported in @arnilo/prism-work/diagrams (P12)");
      }
    }
    ...
    if (input.length >= 4 && input[0] === 0x50 && input[1] === 0x4b && input[2] === 0x03 && input[3] === 0x04) {
      ...
      if (headerSample.includes("visio/") || headerSample.includes("Visio") || headerSample.includes("drawing.xml")) {
        throw new DiagramsFormatError("Visio OpenXML format (.vsdx) is excluded and unsupported in @arnilo/prism-work/diagrams (P12)");
      }
    }
  }
  ...
  if (
    ...
    throw new DiagramsFormatError("Visio format (.vsd/.vsdx) is excluded and unsupported in @arnilo/prism-work/diagrams (P12)");
  }
}
