...
describe("generateDocument", () => {
  ...
  it("round-trips decimal cell strings without Number() coercion", async () => {
    const model: SheetModel = {
      kind: "sheet",
      modelVersion: 1,
      sheets: [{ name: "s", cells: [[{ type: "decimal", value: "1500000.00" }]] }],
    };
    const { bytes } = await generateDocument(model, { format: "xlsx" });
    const parsed = (await parseDocument(bytes, { kind: "sheet" })) as SheetModel;
    assert.deepEqual(parsed.sheets[0]?.cells[0]?.[0], { type: "decimal", value: "1500000.00" });
  });

  it("emits list numbering without a bullet prefix in w:t", async () => {
    const { bytes } = await generateDocument(
      {
        ...
        blocks: [
          ...
          { type: "list", ordered: true, items: ["Beta"] },
        ],
      },
      { format: "docx" },
    );
    const parsed = parseDocumentSync(bytes);
    const paras: ParagraphOptions[] = [];
    for (const section of parsed.sections ?? []) {
      for (const child of section.children ?? []) {
        if (child && typeof child === "object" && "paragraph" in child && child.paragraph && typeof child.paragraph === "object") {
          paras.push(child.paragraph);
        }
      }
    }
    const texts = paras.map((paragraph) => paragraph.text ?? "");
    assert.ok(texts.includes("Alpha"));
    assert.ok(texts.includes("Beta"));
    assert.equal(
      texts.some((text) => text.includes("•") || /^\d+\.\s/.test(text)),
      false,
    );
    assert.ok(paras.every((paragraph) => paragraph.numbering || paragraph.bullet));
  });
});
