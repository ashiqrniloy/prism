...
function listItemChildren(item: string | ListItem) {
  if (typeof item === "string") return [{ text: item }];
  if (item.runs && item.runs.length > 0) return item.runs.map(docRunToRunOptions);
...
function blockToSectionChildren(block: DocBlock): SectionChild[] {
  switch (block.type) {
    ...
    case "list": {
      const numbering = { reference: block.ordered ? "ordered" : "bullet", level: 0 };
...
export function docModelToDocxOptions(model: DocModel): DocumentOptions {
  ...
  return {
    ...(model.blocks.some((block) => block.type === "list")
      ? {
          numbering: {
            abstractNumberings: [
              { reference: "bullet", instanceCount: 1, levels: [{ level: 0, format: "bullet" as const, text: "●" }] },
              { reference: "ordered", instanceCount: 1, levels: [{ level: 0, format: "decimal" as const, text: "%1." }] },
            ],
          },
        }
      : {}),
