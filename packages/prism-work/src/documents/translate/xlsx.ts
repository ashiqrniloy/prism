...
/** Draft-07 DecimalCellValue pattern; requires a fractional part so years/ids stay strings. */
    ...
    for (const row of ws.rows ?? []) {
      ...
      for (const c of row.cells ?? []) {
        if (c.formula && typeof c.formula === "object" && "formula" in c.formula && typeof c.formula.formula === "string") {
          ...
          rowCells.push(CANONICAL_DECIMAL.test(c.value) ? { type: "decimal", value: c.value } : c.value);
