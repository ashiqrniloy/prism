...
import { createXaiOAuthProvider, createXaiProvider, xaiModels } from "../index.js";
...
describe("@arnilo/prism-provider-xai live tests", () => {
  ...
  it("live_supergrok_device_code_login_is_operator_only", { skip: oauthSkip }, async () => {
    ...
    const credentials = await oauth.login({
      onDeviceCode: ({ userCode, verificationUri }) => {
        console.error(`SuperGrok login: open ${verificationUri} and enter ${userCode}`);
      },
    });
    assert.ok(credentials.access, "SuperGrok login returned no access token");
  });
});
