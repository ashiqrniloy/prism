...
describe("@arnilo/prism-providers/openrouter", () => {
  ...
  it("openrouter_passes_provider_routing_reasoning_and_max_tokens", async () => {
    ...
    await assertProviderStreamConforms({
      provider,
      request: { ...request, model: { ...request.model, parameters: { maxTokens: 222, temperature: 0.3 } } },
    });
  ...
  it("openrouter_keeps_provider_owned_headers_after_caller_headers", async () => {
    ...
    assertProviderOwnedHeadersWin(headers, {
      owned: {
        ...
        "x-session-id": "session-with-spaces",
  ...
  it("openrouter_maps_cache_read_write_usage", async () => {
    ...
    const events = await assertProviderStreamConforms({
      provider,
      request,
      expect: { text: "hi", usage: { inputTokens: 5, outputTokens: 2, totalTokens: 7, cacheReadTokens: 1, cacheWriteTokens: 3 } },
    });
  ...
  it("openrouter_redacts_api_key_from_errors", async () => {
    const provider = createOpenRouterProvider({
      apiKey: "fake-openrouter-key",
      fetch: (async () => new Response("bad fake-openrouter-key", { status: 500 })) as typeof fetch,
    });
