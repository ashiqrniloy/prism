...
describe("@arnilo/prism-providers/opencode-go", () => {
  it("opencode_go_registers_official_featured_models_and_setup_does_not_fetch", async () => {
    ...
    const models = registered.filter(
      (item: any): item is ModelConfig => typeof item?.model === "string" && item.provider === "opencode-go",
    );
  ...
  it("opencode_go_session_id_prefers_cacheKey_over_sessionId_and_sanitizes", async () => {
    ...
    await assertProviderStreamConforms({
      provider,
      request: { ...baseRequest, options: { ...baseRequest.options, cacheKey: "repo#prefix/run-1", sessionId: "should-not-win" } },
    });
    assert.equal(headers.get("x-opencode-session"), "repo-prefix-run-1");
    let headers2 = new Headers();
    const provider2 = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async (_input, init) => {
      ...
      request: {
        ...
        options: {
          ...
          headers: {
            ...
            "x-opencode-session": "attacker-session",
            "x-caller": "kept",
          },
        },
      },
  ...
  it("opencode_go_discovery_capability_fields_override_static_verified_set", () => {
    ...
    assert.equal(
      mapOpenCodeGoModel({ id: "deepseek-v4-pro", capabilities: { structured_output: "json_schema" } }).capabilities?.structuredOutput,
      "json_schema",
    );
    assert.equal(
      mapOpenCodeGoModel({ id: "future-model-1", capabilities: { structured_output: true } }).capabilities?.structuredOutput,
      "json_schema",
    );
    // Gateway-authoritative negative: explicit false overrides the static set.
    assert.equal(
      mapOpenCodeGoModel({ id: "mimo-v2.5", capabilities: { structured_output: false } }).capabilities?.structuredOutput,
      undefined,
    );
  ...
  it("opencode_go_deepseek_rejects_structured_output_before_dispatch_and_omits_response_format", async () => {
    ...
    let body: any;
    const plain = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async (_url, init) => {
        body = JSON.parse(String(init?.body));
        return ok(sse([]));
  ...
  it("opencode_go_openai_route_fails_truncated_stream_without_done_or_finish_reason", async () => {
    ...
    const provider = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async () =>
        ok(sseRaw([...Array.from({ length: 36 }, () => ({ choices: [{ delta: { content: "partial " } }] }))]))) as typeof fetch,
    });
  ...
  it("opencode_go_openai_route_fails_done_marker_without_finish_reason", async () => {
    const provider = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async () => ok(sse([{ choices: [{ delta: { content: "partial " } }] }]))) as typeof fetch,
    });
    const events = await assertProviderStreamConforms({ provider, request: baseRequest });
    assert.equal(events.at(-1)?.type, "error");
    assert(!events.some((e) => e.type === "done"));
  ...
  it("opencode_go_openai_route_fails_dangling_tool_call_and_succeeds_when_complete", async () => {
    const dangling = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async () =>
        ok(
          sse([
    ...
    const complete = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async () =>
        ok(
          sse([
  ...
  it("opencode_go_anthropic_route_fails_unclosed_tool_block_and_succeeds_when_complete", async () => {
    const unclosed = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async () =>
        ok(
          sseRaw([
  ...
  it("opencode_go_anthropic_route_error_redacts_key_from_both_auth_headers", async () => {
    const provider = createOpenCodeGoProvider({
      apiKey: "canary-opencode-key",
      fetch: (async () => new Response("unauthorized canary-opencode-key", { status: 401 })) as typeof fetch,
    });
    const events: ProviderEvent[] = [];
  ...
  it("opencode_go_openai_route_serializes_max_tokens_and_temperature", async () => {
    ...
    await assertProviderStreamConforms({
      provider,
      request: { ...baseRequest, model: { ...baseRequest.model, parameters: { maxTokens: 111, temperature: 0.1 } } },
    });
    assert.equal(url.endsWith("/chat/completions"), true);
  ...
  it("opencode_go_openai_and_anthropic_routes_cover_tool_result_replay", async () => {
    ...
    await assertProviderStreamConforms({
      provider: anthropic,
      request: { ...replayAnthropic, model: { ...replayAnthropic.model, parameters: { maxTokens: 222 } } },
    });
  ...
  it("opencode_go_redacts_api_key_from_errors", async () => {
    const provider = createOpenCodeGoProvider({
      apiKey: "fake-opencode-key",
      fetch: (async () => new Response("bad fake-opencode-key", { status: 500 })) as typeof fetch,
    });
    const events = await assertProviderStreamConforms({ provider, request: baseRequest });
    assert.equal(events.at(-1)?.type, "error");
