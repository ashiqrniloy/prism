...
export function hyperChatBody(request: ProviderRequest): JsonObject {
  return buildOpenAIChatBody(request, {
    mapMessages: (req) => req.messages,
    serializeMessage: (message) => serializeHyperChatMessage(message, hyperPreserveThinking(request)),
    transformBody: (body, req) => hyperChatTransform(body, req),
  });
}

function hyperChatTransform(body: JsonObject, request: ProviderRequest): JsonObject {
  const { maxTokens, ...rest } = body as Record<string, unknown>;
  const transformed: Record<string, unknown> = {
    ...rest,
    max_tokens: maxTokens,
    ...stripHyperOwnedCompat(request.options?.compat),
    // Resolved thinking fields win over raw compat passthrough.
    thinking: hyperThinking(request),
    reasoning_effort: hyperReasoningEffort(request),
  };
  applyOpenAIChatStructuredOutput(transformed, request.options?.structuredOutput);
  return clean(transformed);
}

export function hyperChatEvents(body: ReadableStream<Uint8Array>, signal?: AbortSignal): AsyncIterable<ProviderEvent> {
 ...
 * continuity and cache-prefix stability on reasoning models. Image blocks map to
 * `image_url` data URLs for vision models; media-only blocks (audio/document/
 * file) are rejected on this route.
 */
export function serializeHyperChatMessage(message: Message, preserveThinking: boolean): JsonObject {
  ...
  for (const part of message.content) {
    if (part.type === "text") {
      ...
      if (!url) throw new Error("Hyper image block missing url or data");
      content.push({ type: "image_url", image_url: { url } });
    } else if (part.type === "audio" || part.type === "file" || part.type === "document") {
      throw new Error(`Hyper OpenAI route does not support ${part.type} content blocks`);
    } else if (part.type === "tool_call") {
      throw new Error("Hyper assistant tool_call blocks must be serialized with other assistant content");
    } else if (part.type === "tool_result") {
      throw new Error("Hyper tool_result blocks must appear in role=tool messages");
    }
  }
