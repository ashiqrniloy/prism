...
/**
 * Shared Anthropic Messages route serializer (body + SSE events) for the
 * `@arnilo/prism-providers` dual-route adapters (opencode-go pattern).
 *
 * Provider-specific behavior is injected through {@link AnthropicMessagesRouteHooks}
 * so the serializer itself stays provider-agnostic. Existing provider packages
 * (kimi, opencode-go, anthropic) keep their private copies; new adapters
 * (hyper, commandcode) import this module. See
 * `docs/_evidence/phase55-primitive-review.md` for the extraction record.
 */
export interface AnthropicMessagesRouteHooks {
  /** Applies the provider's cache-control markers and returns the messages to serialize. */
  readonly applyCacheControl: (request: ProviderRequest) => readonly CacheControlledMessage[];
  /** Whether assistant thinking blocks are preserved on the wire. */
...
export async function anthropicMessagesBody(request: ProviderRequest, hooks: AnthropicMessagesRouteHooks): Promise<JsonObject> {
  ...
  const preserveThinking = hooks.preserveThinking(request);
...
async function toMessage(
  ...
  for (const part of message.content) {
    ...
    if (part.type === "text") {
      ...
      throw new Error("Anthropic route tool_result blocks must appear in role=tool messages");
    }
  }
