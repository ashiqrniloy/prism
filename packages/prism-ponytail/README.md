...
- Optional peer `@dietrichgebert/ponytail@^4.9.0`, **or** host-supplied `upstreamPath` to a Ponytail checkout with `skills/` and `hooks/`.
...
Commands: `ponytail` (empty/`status` report mode; `lite|full|ultra|off`; `default <mode>`), `ponytail-review`, `ponytail-audit`, `ponytail-gain`, `ponytail-debt`, `ponytail-help`.
