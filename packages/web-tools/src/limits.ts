...
export function resolveWebLimits(input: WebLimits = {}): ResolvedWebLimits {
  ...
  for (const key of Object.keys(DEFAULT_WEB_LIMITS) as (keyof ResolvedWebLimits)[]) {
    ...
    if (!Number.isSafeInteger(value) || value < (permitsZero ? 0 : 1) || value > HARD_WEB_LIMITS[key])
      throw new RangeError(
        `${key} must be ${permitsZero ? "a non-negative" : "a positive"} safe integer no greater than ${HARD_WEB_LIMITS[key]}`,
      );
    out[key] = value;
  }
