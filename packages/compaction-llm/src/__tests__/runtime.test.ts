...
test("llm_compaction_extension_registers_strategy_in_contribution_registry", async () => {
  ...
  await kernel.load([
    createLlmCompactionExtension({ provider: createMockProvider([providerTextDelta("summary"), providerDone()]), model: summaryModel }),
  ]);
...
test("session_manual_compact_with_llm_strategy_appends_compaction_entry", async () => {
  ...
  const session = createAgent({ model, provider: createMockProvider([providerTextDelta("reply"), providerDone()]), store }).createSession({
    id: "s1",
  });
  ...
  const result = await session.compact({
    strategy: createLlmCompactionStrategy({
  ...
  assert.equal(
    (await events).some((event) => event.type === "compaction_finished" && /compact summary/.test(event.summary)),
    true,
  );
});
...
test("llm_compaction_includes_branch_summary_entries_in_summary_input", async () => {
  ...
  const message = createSessionEntry({
    id: "u2",
    parentId: "sum1",
    sessionId: "s1",
    kind: "message",
    message: { role: "user", content: [{ type: "text", text: "recent" }] },
  });
  const strategy = createLlmCompactionStrategy({
    provider: createMockProvider([providerTextDelta("compact summary"), providerDone()], {
      onRequest: (value) => {
...
test("llm_compaction_abort_appends_no_entry", async () => {
  ...
  await assert.rejects(
    session.compact({
      strategy: createLlmCompactionStrategy({ provider: createMockProvider([providerTextDelta("unused")]), model: summaryModel }),
...
test("llm_compaction_runtime_redacts_secret_from_events_and_store", async () => {
  ...
  await session.compact({
    strategy: createLlmCompactionStrategy({
      provider: createMockProvider([providerTextDelta(`summary ${secret}`), providerDone()]),
      model: summaryModel,
      keepRecentTokens: 1,
    }),
    secrets: [secret],
