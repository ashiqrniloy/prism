...
function toolEntry(id: string, parentId?: string): SessionEntry {
  return createSessionEntry({
    ...
    timestamp,
    kind: "message",
    message: { role: "tool", content: [{ type: "tool_result", toolCallId: "call-1", name: "read", result: "ok" }] },
  });
}
...
test("prepare_llm_compaction_keeps_recent_tokens_and_sets_boundary_data", () => {
  ...
  assert.deepEqual(
    prep.entriesToSummarize.map((entry) => entry.id),
    ["u1", "a2"],
  );
  assert.deepEqual(
    prep.entriesToKeep.map((entry) => entry.id),
    ["u3", "a4"],
  );
...
test("prepare_llm_compaction_does_not_cut_at_tool_result", () => {
  const entries = [
    ...
    createSessionEntry({
      ...
      timestamp,
      kind: "message",
      message: { role: "assistant", content: [{ type: "tool_call", id: "call-1", name: "read", arguments: { path: "src/a.ts" } }] },
    }),
    toolEntry("t3", "a2"),
  ];
...
test("prepare_llm_compaction_uses_previous_summary_on_repeated_compaction", () => {
  ...
  const entries = [
    textEntry("u1", "user", "old"),
    textEntry("a2", "assistant", "kept", "u1"),
    previous,
    textEntry("u4", "user", "new", "c3"),
  ];
...
test("prepare_llm_compaction_detects_split_turn_prefix", () => {
  ...
  assert.deepEqual(
    prep.turnPrefixEntries.map((entry) => entry.id),
    ["u2"],
  );
  assert.deepEqual(
    prep.entriesToKeep.map((entry) => entry.id),
