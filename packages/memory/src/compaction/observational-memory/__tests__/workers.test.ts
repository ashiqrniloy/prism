...
const source: SessionEntry = {
  ...
  kind: "message",
  message: { role: "user", content: [{ type: "text", text: "Keep it package-only." }] },
};
...
describe("observational memory workers", () => {
      ...
      provider: provider([
        providerToolCall(
          toolCallContent("c1", "record_observation", {
            content: "User wants package-only memory.",
            relevance: "high",
            sourceEntryIds: ["m1", "invented"],
          }),
        ),
        providerDone(),
      ]),
  ...
  it("reflector_records_reflections_with_valid_support_ids_and_coverage_context", async () => {
    ...
    const reflections = await runReflector({
      ...
      provider: provider([
        providerToolCall(
          toolCallContent("c1", "record_reflection", {
            content: "Keep it optional.",
            supportingObservationIds: [observation.id, "bbbbbbbbbbbb"],
          }),
        ),
        providerDone(),
      ]),
  ...
  it("dropper_records_safe_drops_after_pool_pressure", async () => {
    ...
    const dropped = await runDropper({
      ...
      targetTokens: 1,
      provider: provider([
        providerToolCall(toolCallContent("c1", "drop_observations", { observationIds: [observation.id, "bad"] })),
        providerDone(),
      ]),
      model,
      maxTurns: 1,
  ...
  it("rejects unknown, per-turn, total, argument, result, and message overflow", async () => {
    await assert.rejects(
      runObserver({
        entries: [source],
        provider: provider([providerToolCall(toolCallContent("c1", "unknown", {})), providerDone()]),
        model,
        maxTurns: 1,
      }),
    ...
    await assert.rejects(
      runObserver({
        entries: [source],
        provider: provider([
          providerToolCall(toolCallContent("c1", "record_observation", { content: "x".repeat(100), sourceEntryIds: ["m1"] })),
        ]),
        model,
        maxTurns: 1,
        maxArgumentBytes: 32,
    ...
    const hugeResult: ToolDefinition = {
      name: "store",
      execute: (_args, context) => ({ toolCallId: context.toolCallId, name: "store", value: { text: "x".repeat(100) } }),
    };
    await assert.rejects(
      runMemoryWorkerLoop({
        provider: provider([providerToolCall(toolCallContent("c1", "store", {}))]),
        model,
        system: "s",
        prompt: "p",
        tools: [hugeResult],
    ...
    await assert.rejects(
      runMemoryWorkerLoop({
        provider: provider([providerDone()]),
        model,
        system: "s",
        prompt: "x".repeat(100),
        tools: [],
  ...
  it("redacts bounded provider errors, tool failures, and replayed tool results", async () => {
    ...
    const tool: ToolDefinition = {
      name: "store",
      execute: (_args, context) => ({ toolCallId: context.toolCallId, name: "store", value: { text: secret }, error: { message: secret } }),
    };
    ...
    await assert.rejects(
      runMemoryWorkerLoop({
        provider: provider([providerToolCall(toolCallContent("c1", "throw", {}))]),
        model,
        system: "s",
        prompt: "p",
        tools: [throwing],
  ...
  it("observes abort before retaining provider output", async () => {
    ...
    await assert.rejects(
      runObserver({ entries: [source], provider: provider([providerDone()]), model, maxTurns: 1, signal: controller.signal }),
      /stop/,
    );
  });
