...
export interface AgentEventRow<R = boolean> {
  readonly id: string;
  readonly session_id: string;
  readonly run_id: string | null;
...
export interface ToolCallRow<R = boolean> {
  readonly id: string;
  readonly session_id: string;
  readonly run_id: string | null;
  readonly entry_id: string | null;
...
export interface SessionRowMappers<R> {
  sessionEntryToRow(entry: SessionEntry): SessionEntryRow;
  rowToSessionEntry(row: SessionEntryRow): SessionEntry;
  runRecordToRow(record: RunRecord): RunRow;
  rowToRunRecord(row: RunRow): RunRecord;
  agentEventRecordToRow(record: AgentEventRecord, sequence: number): AgentEventRow<R>;
  rowToAgentEventRecord(row: AgentEventRow<R>): AgentEventRecord;
  toolCallRecordToRow(record: ToolCallRecord): ToolCallRow<R>;
  rowToToolCallRecord(row: ToolCallRow<R>): ToolCallRecord;
  usageRecordToRow(record: UsageRecord): UsageRow;
  rowToUsageRecord(row: UsageRow): UsageRecord;
  rowToSessionRecord(row: Record<string, unknown>): SessionRecord;
  rowToBranchRecord(row: Record<string, unknown>): BranchRecord;
  rowToAgentDefinitionRecord(row: Record<string, unknown>): AgentDefinitionRecord;
  rowToRetentionPolicy(row: Record<string, unknown>): RetentionPolicy;
  rowToMigrationRecord(row: Record<string, unknown>): MigrationRecord;
  encodeEntryCursor(timestamp: string, id: string): string;
  decodeEntryCursor(cursor: string): { timestamp: string; id: string };
  parentKey(parentId: string | undefined): string;
}

/** Shared session-store row codecs; the only store-specific seam is the `redacted` boolean representation. */
export function createSessionRowMappers<R>(redacted: RedactedCodec<R>): SessionRowMappers<R> {
  return {
    sessionEntryToRow(entry: SessionEntry): SessionEntryRow {
      return {
        id: entry.id,
    ...
    runRecordToRow(record: RunRecord): RunRow {
      return {
        ...
        prompt_version: stringifyJson(record.promptVersion),
      };
    },
    rowToRunRecord(row: RunRow): RunRecord {
      return {
        ...
        metadata: parseJson(row.metadata),
        promptVersion: parseJson(row.prompt_version),
      };
    },
    agentEventRecordToRow(record: AgentEventRecord, sequence: number): AgentEventRow<R> {
      return {
        ...
        redacted: redacted.encode(record.redacted),
        tenant_id: record.tenantId ?? null,
        account_id: record.accountId ?? null,
        user_id: record.userId ?? null,
        metadata: stringifyJson(record.metadata),
    ...
    rowToAgentEventRecord(row: AgentEventRow<R>): AgentEventRecord {
      return {
        ...
        redacted: redacted.decode(row.redacted),
        tenantId: row.tenant_id ?? undefined,
        accountId: row.account_id ?? undefined,
        userId: row.user_id ?? undefined,
        metadata: parseJson(row.metadata),
