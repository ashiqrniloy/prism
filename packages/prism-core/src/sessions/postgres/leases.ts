...
export function createPostgresLeaseStore(pool: Pool, schema = "prism"): LeaseStore {
  ...
  const load = async (namespace: string, key: string, activeOnly = false): Promise<LeaseRecord | null> => {
    ...
    const result = await pool.query(
      `SELECT * FROM ${table} WHERE namespace=$1 AND key=$2${activeOnly ? " AND expires_at > CURRENT_TIMESTAMP" : ""}`,
      [namespace, key],
    );
...
RETURNING *`,
      ...
      if (current) assertOwnershipScope(input, current, () => new LeaseConflictError("Lease ownership mismatch"));
      return null;
    },
...
WHERE namespace=$1 AND key=$2 AND owner_id=$3 AND token=$4
  AND tenant_id IS NOT DISTINCT FROM $5 AND account_id IS NOT DISTINCT FROM $6 AND user_id IS NOT DISTINCT FROM $7`,
        [input.namespace, input.key, input.ownerId, input.token, input.tenantId ?? null, input.accountId ?? null, input.userId ?? null],
      );
