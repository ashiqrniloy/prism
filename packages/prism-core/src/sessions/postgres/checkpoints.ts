...
export function createPostgresCheckpointStore(pool: Pool, schema = "prism"): CheckpointStore {
  ...
  const ensureReady = () =>
    (ready ??= (async () => {
      ...
      await pool.query(
        `CREATE INDEX IF NOT EXISTS prism_checkpoints_list_idx ON ${table} (namespace, category, tenant_id, updated_at DESC, key)`,
      );
    })());
...
RETURNING *`,
      ...
      if (result.rowCount === 0) throw staleCheckpoint(input.version, previous?.version);
    ...
    async listCheckpoints(query: CheckpointQuery = {}) {
      ...
      const result = await pool.query(
        `SELECT * FROM ${table} ${clauses.length ? `WHERE ${clauses.join(" AND ")}` : ""} ORDER BY updated_at DESC, key ASC LIMIT $${params.length - 1} OFFSET $${params.length}`,
        params,
      );
      const rows = result.rows as Row[];
      return {
        items: rows.slice(0, limit).map((row) => rowToRecord(row)!),
