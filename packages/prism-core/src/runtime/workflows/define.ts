  ...
  for (const [nodeId, node] of Object.entries(input.nodes)) {
    ...
    if (node.kind === "loop") {
      if (node.maxIterations === undefined) {
        throw new WorkflowDefinitionError(`Loop node "${nodeId}" maxIterations is required`);
      }
      validateWorkflowLimit(`Node "${nodeId}" maxIterations`, node.maxIterations, HARD_MAX_LOOP_ITERATIONS);
      if (typeof node.until !== "function") {
        throw new WorkflowDefinitionError(`Loop node "${nodeId}" requires until()`);
      }
      const hasExecute = typeof node.execute === "function";
      const hasBody = node.body !== undefined;
      if (hasExecute === hasBody) {
        throw new WorkflowDefinitionError(`Loop node "${nodeId}" requires exactly one of execute or body`);
      }
      if (hasBody) {
        if (node.body?.kind === "function" && typeof node.body.execute !== "function") {
          throw new WorkflowDefinitionError(`Loop node "${nodeId}" body requires execute()`);
        }
        if (node.body?.kind === "tool" && typeof node.body.args !== "function") {
          throw new WorkflowDefinitionError(`Loop node "${nodeId}" tool body requires args()`);
        }
        if (node.body?.kind !== "function" && node.body?.kind !== "tool") {
          throw new WorkflowDefinitionError(`Loop node "${nodeId}" body must be a function or tool node`);
        }
      }
    }
  }
