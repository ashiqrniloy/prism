  ...
  return {
    ...
    async load(input): Promise<WorkflowCheckpointRecord | null> {
      try {
        ...
        return record ? toWorkflowRecord(record, limits.maxNodeOutputBytes) : null;
      } catch (error) {
        throw asWorkflowCheckpointError(error);
      }
    },
    ...
    async list(input: WorkflowCheckpointListInput = {}): Promise<WorkflowCheckpointListPage> {
      ...
      try {
        ...
        return { items: page.items.map((record) => toWorkflowRecord(record, limits.maxNodeOutputBytes)), nextCursor: page.nextCursor };
      } catch (error) {
        throw asWorkflowCheckpointError(error);
      }
    },
...
function toWorkflowRecord(record: CheckpointRecord, maxNodeOutputBytes: number): WorkflowCheckpointRecord {
  ...
  return {
    ...
    value: parseCheckpointValue(record.value, maxNodeOutputBytes),
...
export function redactCheckpointOutputs(
  ...
  for (const [id, node] of Object.entries(value.nodes)) {
    nodes[id] = {
      ...node,
      ...(node.output === undefined ? {} : { output: redactValue(node.output, redactor) }),
      ...(node.lastOutput === undefined ? {} : { lastOutput: redactValue(node.lastOutput, redactor) }),
      ...(node.iterations === undefined
        ? {}
        : {
            iterations: node.iterations.map((iteration) => ({
              ...iteration,
              ...(iteration.output === undefined ? {} : { output: redactValue(iteration.output, redactor) }),
            })),
          }),
    };
  }
