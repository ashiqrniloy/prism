...
export async function deliverA2APushEvent(
  delivery: A2APushDelivery,
  config: A2APushConfig,
  event: A2ATaskEvent,
  options: DeliverA2APushEventOptions = {},
  ...
  throw new A2AError(
    last instanceof DOMException && last.name === "AbortError" ? "A2A push delivery timed out" : "A2A push delivery failed",
    502,
    "ERR_PRISM_A2A_PUSH",
  );
}
