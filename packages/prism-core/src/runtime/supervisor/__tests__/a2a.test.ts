...
const rpc = (method = "SendMessage", input = "question") => ({
  jsonrpc: "2.0",
  id: 1,
  method,
  params: { message: { role: "user", messageId: "m1", parts: [{ text: input }] } },
});
const streamEvent = (state: string, text?: string) =>
  JSON.stringify({
    jsonrpc: "2.0",
    id: 1,
    result: {
      task: {
        ...
        contextId: "c1",
        status: { state },
        artifacts: text === undefined ? [] : [{ artifactId: "a1", parts: [{ text }] }],
      },
    },
  });
...
function streamClient(chunks: readonly Uint8Array[], limits?: A2ALimits) {
  return createA2AClient({
    ...
    limits,
    fetch: async (input) =>
      new URL(String(input)).pathname.includes("well-known")
...
describe("A2A agent cards", () => {
  it("canonicalizes, signs, verifies, and rejects tamper/expiry", async () => {
    ...
    const signed = await signA2AAgentCard(card, {
    ...
    await assert.rejects(
      verifyA2AAgentCard({ ...signed, description: "tampered" }, { publicKey: keys.publicKey, now: new Date("2026-06-01T00:00:00Z") }),
      /invalid or expired/,
    );
    await assert.rejects(
...
describe("createA2AHandler", () => {
  it("serves discovery and authorized SendMessage", async () => {
    ...
    const response = await handler(
      new Request(endpoint, { method: "POST", headers: { "content-type": "application/a2a+json" }, body: JSON.stringify(rpc()) }),
    );
  ...
  it("fails closed on auth, unsupported parts/methods, and request limits", async () => {
    assert.throws(
      () =>
        createA2AHandler({
          card: { ...baseCard(), capabilities: { streaming: true, pushNotifications: true } },
          exposure: { sessionFactory: () => session() },
          authorize: () => ({ ownership }),
        }),
      /push capability/,
    );
    const denied = createA2AHandler({
      card: baseCard(),
      exposure: { sessionFactory: () => session() },
      authorize: () => false,
    ...
    const unauthorized = await denied(
      new Request(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(rpc()) }),
    );
    assert.equal(unauthorized.status, 403);
    const allowed = createA2AHandler({
      card: baseCard(),
      exposure: { sessionFactory: () => session() },
      authorize: () => ({ ownership }),
      limits: { maxRequestBytes: 256 },
    ...
    assert.equal(
      (await allowed(new Request(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(bad) })))
        .status,
      400,
    );
    const unknown = await allowed(
      new Request(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(rpc("Unknown")) }),
    );
      ...
      (
        await allowed(
          new Request(endpoint, {
            ...
            body: JSON.stringify(rpc("SendMessage", "x".repeat(300))),
          }),
        )
      ).status,
      413,
  ...
  it("supports durable rich tasks, owner-hidden lookup, replay subscriptions, and host-owned push configs", async () => {
    ...
    const lifecycle: A2ATaskLifecycle = {
      async start({ message, authorization }) {
        const task: A2ATask = {
          id: "durable-1",
          contextId: message.contextId ?? "ctx-1",
          status: { state: "TASK_STATE_INPUT_REQUIRED", timestamp: new Date().toISOString() },
          artifacts: [{ artifactId: "rich", parts: message.parts }],
        };
        tasks.set(`${authorization.ownership.userId}:${task.id}`, task);
        return task;
      },
      async get({ id, authorization }) {
        return tasks.get(`${authorization.ownership.userId}:${id}`);
      },
      async list({ authorization }) {
        return {
          tasks: [...tasks.entries()].filter(([key]) => key.startsWith(`${authorization.ownership.userId}:`)).map(([, task]) => task),
        };
      },
      async cancel({ id, authorization }) {
        const task = tasks.get(`${authorization.ownership.userId}:${id}`);
        return task && { ...task, status: { state: "TASK_STATE_CANCELED", timestamp: new Date().toISOString() } };
      },
      async *subscribe({ id, afterEventId, authorization }) {
        replayCursor = afterEventId;
        const task = tasks.get(`${authorization.ownership.userId}:${id}`);
        if (task) yield { eventId: "event-2", task };
      },
    };
    ...
    const handler = createA2AHandler({
      card: { ...baseCard(), capabilities: { streaming: true, pushNotifications: true } },
      exposure: { sessionFactory: () => session() },
      authorize: ({ request }) => ({ ownership: { tenantId: "tenant", userId: request.headers.get("x-user") ?? "user" } }),
      tasks: lifecycle,
      push,
      parts: {
        allowRaw: true,
        ...
        allowUrl: true,
        validateUrl: (url) => {
          if (url.hostname !== "files.example") throw new Error("blocked URL");
        },
      },
    });
    const call = async (method: string, params: object, user = "user") =>
      handler(
        new Request(endpoint, {
          method: "POST",
          headers: { "content-type": "application/a2a+json", "x-user": user },
    ...
    const created = await (
      await call("SendMessage", {
        message: {
          role: "ROLE_USER",
          messageId: "m-rich",
          parts: [{ raw: "aGk=", mediaType: "text/plain" }, { data: { safe: true } }, { url: "https://files.example/report.pdf" }],
        },
      })
    ).json();
    assert.equal(created.result.task.status.state, "TASK_STATE_INPUT_REQUIRED");
    const malformedRaw = await call("SendMessage", { message: { role: "ROLE_USER", messageId: "bad-raw", parts: [{ raw: "%%%" }] } });
    assert.equal(malformedRaw.status, 400);
    let nested: any = null;
    for (let i = 0; i < 70; i++) nested = { nested };
    const deepData = await call("SendMessage", { message: { role: "ROLE_USER", messageId: "deep", parts: [{ data: nested }] } });
    assert.equal(deepData.status, 400);
    ...
    assert.match(await subscribed.text(), /event-2/);
    assert.equal(replayCursor, "event-1");
    const pushed = await (
      await call("CreateTaskPushNotificationConfig", {
        taskId: "durable-1",
        config: {
          id: "push-1",
          url: "https://files.example/hook",
          token: "secret",
          authentication: { scheme: "Bearer", credentials: "secret" },
        },
      })
    ).json();
    assert.equal(pushed.result.id, "push-1");
    assert.equal(pushed.result.token, undefined);
    assert.equal(pushed.result.authentication.credentials, undefined);
    const deliveries: string[] = [];
    const delivery = await deliverA2APushEvent(
      {
        async deliver(input) {
          deliveries.push(`${input.idempotencyKey}:${input.attempt}`);
          if (input.attempt === 1) throw new Error("retry");
        },
      },
      configs.get("push-1")!,
      { eventId: "event-push", task: tasks.get("user:durable-1")! },
      { maxAttempts: 2 },
    );
    assert.equal(delivery.attempts, 2);
    assert.deepEqual(deliveries, ["event-push:1", "event-push:2"]);
    const client = createA2AClient({
      endpoint,
      allowedOrigins: ["https://agent.example"],
      fetch: (input, init) => handler(new Request(input, init)),
      authorize: () => ({ "x-user": "user" }),
    });
    assert.equal(
      (await client.sendMessage({ role: "ROLE_USER", messageId: "client-rich", parts: [{ data: { from: "client" } }] })).status.state,
      "TASK_STATE_INPUT_REQUIRED",
    );
    assert.equal((await client.getTask("durable-1")).id, "durable-1");
    assert.equal((await client.listTasks()).tasks.length, 1);
    const replay: string[] = [];
    for await (const event of client.subscribeToTask("durable-1", { afterEventId: "event-1" })) replay.push(event.eventId);
    assert.deepEqual(replay, ["event-2"]);
    assert.equal((await client.cancelTask("durable-1")).status.state, "TASK_STATE_CANCELED");
    assert.equal((await client.getPushConfig("durable-1", "push-1")).id, "push-1");
    assert.equal((await client.listPushConfigs("durable-1")).configs.length, 1);
    await client.deletePushConfig("durable-1", "push-1");
    assert.equal(
      (
        await handler(
          new Request(endpoint, {
            method: "POST",
            headers: { "content-type": "application/a2a+json", "a2a-version": "2.0" },
  ...
  it("streams bounded working and completed task events with redaction", async () => {
    const handler = createA2AHandler({
      ...
      exposure: { sessionFactory: () => session() },
      authorize: () => ({ ownership }),
      redactor: createSecretRedactor(["answer"]),
    });
    const response = await handler(
      new Request(endpoint, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(rpc("SendStreamingMessage")),
      }),
    );
...
describe("createA2AClient", () => {
  it("discovers, verifies, authenticates after serialization, and maps remote results", async () => {
    const keys = await crypto.subtle.generateKey({ name: "ECDSA", namedCurve: "P-256" }, true, ["sign", "verify"]);
    const card = await signA2AAgentCard(baseCard(), {
      privateKey: keys.privateKey,
      keyId: "key-1",
    ...
    const handler = createA2AHandler({
      card,
      exposure: { sessionFactory: () => session() },
      authorize: ({ request }) => (request.headers.get("authorization") === "Bearer token" ? { ownership } : false),
    });
    let authCalls = 0;
    const client = createA2AClient({
      endpoint,
  ...
  it("rejects malformed UTF-8, truncated frames, post-terminal events, and existing stream limits", async () => {
    const encoder = new TextEncoder();
    await assert.rejects(
      collectStream(streamClient([Uint8Array.from([0x64, 0x61, 0x74, 0x61, 0x3a, 0x20, 0xc3])])),
      /Malformed A2A UTF-8 stream/,
    );
    await assert.rejects(
      collectStream(streamClient([encoder.encode(`data: ${streamEvent("TASK_STATE_COMPLETED")}`)])),
      /Truncated A2A stream/,
    );
    await assert.rejects(
      collectStream(
        streamClient([encoder.encode(`data: ${streamEvent("TASK_STATE_COMPLETED")}\n\ndata: ${streamEvent("TASK_STATE_WORKING")}\n\n`)]),
      ),
      /continued after terminal/,
    );
    await assert.rejects(
      collectStream(streamClient([encoder.encode(`data: ${"x".repeat(64)}\n\n`)], { maxEventBytes: 32 })),
      /event exceeds max bytes/,
    );
    await assert.rejects(
      collectStream(streamClient([encoder.encode(`data: ${streamEvent("TASK_STATE_COMPLETED")}\n\n`)], { maxStreamBytes: 32 })),
      /stream exceeds max bytes/,
    );
    await assert.rejects(
      collectStream(streamClient([encoder.encode(`:\n\ndata: ${streamEvent("TASK_STATE_COMPLETED")}\n\n`)], { maxStreamEvents: 1 })),
      /stream exceeds max events/,
    );
  });
  ...
  it("rejects origins before fetch and malformed/oversized/aborted remote data", async () => {
    ...
    assert.throws(
      () =>
        createA2AClient({
          endpoint: "http://internal/a2a",
          allowedOrigins: ["http://internal"],
          fetch: async () => {
            calls += 1;
            return new Response();
          },
    ...
    const malformed = createA2AClient({
      endpoint,
      allowedOrigins: ["https://agent.example"],
      fetch: async (input) =>
        new URL(String(input)).pathname.includes("well-known") ? Response.json(baseCard()) : Response.json({ bad: true }),
    });
    await assert.rejects(malformed.send("x"), /Malformed A2A JSON-RPC/);
    const oversized = createA2AClient({
      endpoint,
      allowedOrigins: ["https://agent.example"],
      limits: { maxResponseBytes: 16 },
      fetch: async (input) =>
        new URL(String(input)).pathname.includes("well-known") ? Response.json(baseCard()) : Response.json({ value: "x".repeat(100) }),
    });
    await assert.rejects(oversized.send("x"), /exceeds max bytes/);
    const controller = new AbortController();
    const aborted = createA2AClient({
      endpoint,
      allowedOrigins: ["https://agent.example"],
