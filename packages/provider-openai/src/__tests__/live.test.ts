...
import { createOpenAIResponsesProvider, openAIModels } from "../index.js";

// Env-gated live smoke tests for @arnilo/prism-provider-openai.
//
...
//     npm run test --workspace=@arnilo/prism-provider-openai
//
...
const skip: string | false = !LIVE || !API_KEY
  ? "set PRISM_LIVE_PROVIDER_TESTS=1 and OPENAI_API_KEY to run live OpenAI smoke tests"
  : false;
...
describe("@arnilo/prism-provider-openai live tests", () => {
  ...
  it("live_tool_call_loop_conforms_and_leaks_no_secret", { skip }, async () => {
    ...
    // verify it reconstructs to a named call.
