...
export async function attachMcpCapabilities(
  ...
  const maxBytes = validateMcpLimit(
    "maxCapabilityBytes",
    options.maxCapabilityBytes ?? DEFAULT_MAX_CAPABILITY_BYTES,
    HARD_MAX_CAPABILITY_BYTES,
  );
      ...
      const pageResult =
        name === "resources"
          ? await client
              ...
              .then((result) => ({ items: result.resources as readonly unknown[], nextCursor: result.nextCursor, raw: result }))
          : await client
              .listPrompts(cursor ? { cursor } : undefined)
              .then((result) => ({ items: result.prompts as readonly unknown[], nextCursor: result.nextCursor, raw: result }));
      bounded(pageResult.raw, maxBytes, `MCP ${name}/list`);
      if (out.length + pageResult.items.length > DEFAULT_MAX_CAPABILITY_ITEMS)
        throw new McpBridgeError(`MCP ${name}/list exceeds ${DEFAULT_MAX_CAPABILITY_ITEMS} items`);
  ...
  return {
    ...
    async readResource(uri) {
      requireCapability("resources");
      return bounded(await client.readResource({ uri }), maxBytes, "MCP resource result");
    },
    listPrompts: () => collect("prompts"),
    async getPrompt(name, args) {
      requireCapability("prompts");
      return bounded(await client.getPrompt({ name, arguments: args }), maxBytes, "MCP prompt result");
    },
  };
}
...
export function createMcpCapabilityClient(options: ConnectMcpCapabilitiesOptions): Client {
  ...
  if (options.roots)
    client.setRequestHandler("roots/list", async () => {
      ...
      for (const root of roots) {
        let uri: URL;
        try {
          uri = new URL(root.uri);
        } catch {
          throw new McpBridgeError(`Invalid MCP root URI: ${root.uri}`);
        }
  ...
  if (options.elicitation)
    client.setRequestHandler("elicitation/create", async (request, ctx) => {
      ...
      if (marked.action === "accept" && marked.humanInteraction !== true)
