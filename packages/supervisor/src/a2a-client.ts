...
import { type ResolvedA2ALimits, resolveA2ALimits } from "./a2a-parts.js";
import {
...
export function createA2AClient(options: A2AClientOptions): A2AClient {
    ...
    return withRequest(call.signal, async (signal) => {
      ...
      if (
        !card.supportedInterfaces.some(
          (item) => item.protocolBinding === "JSONRPC" && item.protocolVersion === A2A_PROTOCOL_VERSION && item.url === endpoint.href,
        )
      )
  ...
  async function* stream(input: string, call: { readonly signal?: AbortSignal } = {}): AsyncGenerator<string> {
    ...
    try {
      ...
      const authHeaders = await abortable(
        Promise.resolve(options.authorize?.({ endpoint: endpoint.href, signal: owned.signal }) ?? {}),
        owned.signal,
      );
      const response = await fetcher(endpoint, {
        method: "POST",
  ...
  async function sendMessage(
    ...
    call: { readonly signal?: AbortSignal; readonly returnImmediately?: boolean } = {},
  ): Promise<A2ATask> {
    return parseTaskResult(
      await invoke("SendMessage", { message, configuration: { returnImmediately: call.returnImmediately ?? false } }, call.signal),
    );
  }
  async function getTask(id: string, call: { readonly signal?: AbortSignal; readonly historyLength?: number } = {}): Promise<A2ATask> {
    return parseTaskResult(await invoke("GetTask", { id, historyLength: call.historyLength ?? 0 }, call.signal));
  }
  async function listTasks(
    call: { readonly signal?: AbortSignal; readonly pageSize?: number; readonly pageToken?: string; readonly contextId?: string } = {},
  ): Promise<import("./a2a-types.js").A2ATaskPage> {
    const value = await invoke(
      "ListTasks",
      { pageSize: call.pageSize ?? 50, pageToken: call.pageToken, contextId: call.contextId },
      call.signal,
    );
  ...
  async function cancelTask(id: string, call: { readonly signal?: AbortSignal } = {}): Promise<A2ATask> {
    return parseTaskResult(await invoke("CancelTask", { id }, call.signal));
  }
  async function* subscribeToTask(
    id: string,
    call: { readonly signal?: AbortSignal; readonly afterEventId?: string } = {},
  ): AsyncGenerator<import("./a2a-types.js").A2ATaskEvent> {
    if (active >= limits.maxConcurrentRequests) throw new A2AError("A2A client concurrency exceeded", 429, "ERR_PRISM_A2A_CONCURRENCY");
    active += 1;
    const owned = ownedSignal(call.signal, limits.timeoutMs);
    let reader: ReadableStreamDefaultReader<Uint8Array> | undefined;
  ...
  async function createPushConfig(config: import("./a2a-types.js").A2APushConfig, call: { readonly signal?: AbortSignal } = {}) {
    return parsePushConfig(await invoke("CreateTaskPushNotificationConfig", { ...config }, call.signal));
  }
  async function getPushConfig(taskId: string, id: string, call: { readonly signal?: AbortSignal } = {}) {
    return parsePushConfig(await invoke("GetTaskPushNotificationConfig", { taskId, id }, call.signal));
  }
  async function listPushConfigs(
    taskId: string,
    call: { readonly signal?: AbortSignal; readonly pageSize?: number; readonly pageToken?: string } = {},
  ) {
    const value = await invoke(
      "ListTaskPushNotificationConfigs",
      { taskId, pageSize: call.pageSize, pageToken: call.pageToken },
      call.signal,
    );
  ...
  async function deletePushConfig(taskId: string, id: string, call: { readonly signal?: AbortSignal } = {}) {
...
function taskResult(task: A2ATask, options: A2AClientOptions): AgentRunResult {
  ...
  const status =
    task.status.state === "TASK_STATE_COMPLETED" ? "succeeded" : task.status.state === "TASK_STATE_CANCELED" ? "aborted" : "failed";
...
function parseRemoteMessage(value: unknown): import("./a2a-types.js").A2AMessage {
  if (
    !isRecord(value) ||
...
function parsePushConfig(value: unknown): import("./a2a-types.js").A2APushConfig {
  ...
  return {
    ...
    authentication:
      isRecord(value.authentication) && typeof value.authentication.scheme === "string"
        ? {
...
function remoteProtocolError(code: number, message: string, options: A2AClientOptions): A2AError {
  return new A2AError(
    safeRemote(message, options),
    code === -32001 ? 404 : code === -32004 ? 501 : 502,
    code === -32001 ? "ERR_PRISM_A2A_TASK_NOT_FOUND" : code === -32004 ? "ERR_PRISM_A2A_UNSUPPORTED" : "ERR_PRISM_A2A_REMOTE",
  );
}
...
function safeRemote(message: string, options: A2AClientOptions): string {
