import { type AgentRunResult, assertIdentityActive, assertIdentityMatchesOwnership } from "@arnilo/prism";
import { createA2AAgentCard } from "./a2a-card.js";
import {
  bounded,
  optionalCursor,
  parseA2AMessage,
  type ResolvedA2ALimits,
  record,
  requireId,
  resolveA2ALimits,
  validateA2ATask,
    ...
    try {
      ...
      try {
        ...
        const authorization = await abortable(
          Promise.resolve(options.authorize({ request, method: rpc.method, signal: owned.signal })),
          owned.signal,
        );
        ...
        if (rpc.method === "SendMessage" || rpc.method === "SendStreamingMessage") {
          ...
          const taskId = `task-${crypto.randomUUID()}`,
            contextId = message.contextId ?? `context-${crypto.randomUUID()}`;
          const runOptions = {
          ...
          if (rpc.method === "SendMessage")
            return json(
              rpc.id,
              { task: toTask(taskId, contextId, await abortable(session.run(input, runOptions), owned.signal), options) },
              limits,
              options,
            );
          transferred = true;
        ...
        if (["GetTask", "ListTasks", "CancelTask", "SubscribeToTask"].includes(rpc.method)) {
          ...
          if (rpc.method === "ListTasks") {
            const pageSize = integer(rpc.params?.pageSize, 50, limits.maxPageSize),
              pageToken = optionalCursor(rpc.params?.pageToken, limits),
              contextId = rpc.params?.contextId === undefined ? undefined : requireId(rpc.params.contextId, limits, "context id");
          ...
          const taskId = requireId(rpc.params?.id, limits),
            afterEventId = optionalCursor(rpc.params?.afterEventId, limits);
          ...
          return streamResponse(
            rpc.id,
            options.tasks.subscribe({ id: taskId, afterEventId, authorization, signal: owned.signal }),
            owned,
            limits,
            options,
            () => {
...
async function pushOperation(
  ...
  const taskId = requireId(rpc.params?.taskId, limits),
    id = rpc.params?.id === undefined ? undefined : requireId(rpc.params.id, limits, "push config id");
  ...
  if (rpc.method === "ListTaskPushNotificationConfigs") {
    const pageSize = integer(rpc.params?.pageSize, limits.maxPushConfigs, limits.maxPushConfigs),
      pageToken = optionalCursor(rpc.params?.pageToken, limits);
    ...
    return json(
      rpc.id,
      { configs: page.configs.map(publicPush), nextPageToken: optionalCursor(page.nextPageToken, limits) },
      limits,
      options,
    );
  }
...
async function parsePush(
  ...
  const id = requireId(value.id, limits, "push config id"),
    taskId = requireId(value.taskId, limits);
  ...
  const authentication =
    record(value.authentication) && typeof value.authentication.scheme === "string"
      ? {
...
function publicPush(value: A2APushConfig): A2APushConfig {
  return {
    id: value.id,
    taskId: value.taskId,
    url: value.url,
...
function streamResponse(
  ...
  source: AsyncIterable<A2ATaskEvent>,
    ...
    new ReadableStream<Uint8Array>({
      async pull(controller) {
        try {
          ...
          if (
            events > Math.min(limits.maxStreamEvents, limits.maxReplayEvents) ||
...
async function validateTaskEvent(event: A2ATaskEvent, limits: ResolvedA2ALimits, options: CreateA2AHandlerOptions): Promise<void> {
  ...
  if ("statusUpdate" in event) {
    await validateA2ATask(
      { id: event.statusUpdate.taskId, contextId: event.statusUpdate.contextId, status: event.statusUpdate.status },
      limits,
      options.parts,
    );
    return;
...
async function* oneEvent(task: A2ATask): AsyncGenerator<A2ATaskEvent> {
...
function toTask(taskId: string, contextId: string, result: AgentRunResult, options: CreateA2AHandlerOptions): A2ATask {
  const state =
    result.status === "succeeded" ? "TASK_STATE_COMPLETED" : result.status === "aborted" ? "TASK_STATE_CANCELED" : "TASK_STATE_FAILED";
...
function parseRpc(value: unknown): A2AJsonRpcRequest {
  ...
  return { jsonrpc: "2.0", id: value.id, method: value.method, params: value.params as Record<string, unknown> | undefined };
}
...
function json(id: A2AJsonRpcRequest["id"], result: unknown, limits: ResolvedA2ALimits, options: CreateA2AHandlerOptions): Response {
...
function rpcError(id: A2AJsonRpcRequest["id"], code: number, message: string): Response {
...
function errorResponse(status: number, message: string, id: A2AJsonRpcRequest["id"]): Response {
  const body: A2AJsonRpcResponse = { jsonrpc: "2.0", id, error: { code: status === 404 ? -32001 : -32000, message } };
...
function terminal(state: A2ATask["status"]["state"]): boolean {
  return finalTerminal(state) || state === "TASK_STATE_INPUT_REQUIRED" || state === "TASK_STATE_AUTH_REQUIRED";
}
function finalTerminal(state: A2ATask["status"]["state"]): boolean {
