...
export async function validateA2ATask(
  task: A2ATask,
  limits: ResolvedA2ALimits,
  policy: A2APartPolicy = { allowRaw: true, allowUrl: true, allowData: true },
): Promise<A2ATask> {
  if (
    !id(task.id, limits) ||
...
export function requireId(value: unknown, limits: ResolvedA2ALimits, label = "task id"): string {
  if (!id(value, limits)) throw new A2AError(`Invalid A2A ${label}`, 400, "ERR_PRISM_A2A_REQUEST");
  return value;
}
export function optionalCursor(value: unknown, limits: ResolvedA2ALimits): string | undefined {
...
export function bounded<T>(value: T, maxBytes: number, label: string): T {
  ...
  while (stack.length) {
    ...
    if (properties > 10_000) throw new A2AError(`${label} exceeds JSON property limit`, 400, "ERR_PRISM_A2A_REQUEST");
  ...
  if (Buffer.byteLength(json) > maxBytes) throw new A2AError(`${label} exceeds max bytes`, 507, "ERR_PRISM_A2A_RESPONSE_LIMIT");
  return value;
}
export function record(value: unknown): value is Record<string, unknown> {
  return !!value && typeof value === "object" && !Array.isArray(value);
}
function id(value: unknown, limits: ResolvedA2ALimits): value is string {
  return typeof value === "string" && value.length > 0 && Buffer.byteLength(value) <= limits.maxIdBytes;
}
function optionalId(value: unknown, limits: ResolvedA2ALimits): string | undefined {
