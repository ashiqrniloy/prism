...
// Env-gated live smoke tests for @arnilo/prism-provider-zai.
//
// Network-free by default: these tests skip unless BOTH
// `PRISM_LIVE_PROVIDER_TESTS=1` AND `ZAI_API_KEY` are set. The default
// `npm test` and CI release verification never set these. To run locally:
//
//   PRISM_LIVE_PROVIDER_TESTS=1 ZAI_API_KEY=... \
//     npm run test --workspace=@arnilo/prism-provider-zai
//
...
const skip: string | false = !LIVE || !API_KEY
  ? "set PRISM_LIVE_PROVIDER_TESTS=1 and ZAI_API_KEY to run live ZAI smoke tests"
  : false;
...
describe("@arnilo/prism-provider-zai live tests", () => {
