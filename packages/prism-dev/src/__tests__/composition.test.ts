...
describe("composition (plan 040 Task 1)", () => {
  it("imports only public seams: no core internals, allow-listed @arnilo specifiers", () => {
    ...
    const allowList = ["@arnilo/prism", "@arnilo/prism-server", "@arnilo/prism-ag-ui/renderer"];
    assert.deepEqual(
      [...new Set(arnilo)].sort(),
  ...
  it("manifest: peers ^0.3.0, zero runtime dependencies, omitted from umbrellas", () => {
    const manifest = readManifest("package.json");
    assert.equal(manifest.name, "@arnilo/prism-dev");
    assert.equal(manifest.version, "0.0.1");
    assert.deepEqual(manifest.peerDependencies, {
      "@arnilo/prism": "^0.3.0",
      "@arnilo/prism-ag-ui": "^0.3.0",
      "@arnilo/prism-server": "^0.3.0",
    });
    assert.deepEqual(manifest.dependencies, undefined, "composition-only: no runtime dependencies");
    assert.equal(manifest.private, undefined, "must be publishable");
    const prismAll = readManifest("../../packages/prism-all/package.json");
    assert.ok(!JSON.stringify(prismAll.dependencies).includes("@arnilo/prism-dev"), "dev-only package stays out of the prism-all umbrella");
  });
