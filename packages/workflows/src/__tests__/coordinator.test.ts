...
describe("distributed workflow coordinator", () => {
  ...
  it("bounds concurrent claimed runs", async () => {
    ...
    const definition = workflow("bounded", async () => {
      await gate;
      return "done";
    });
    const queued = await Promise.all(
      [0, 1, 2].map((index) => enqueueWorkflow(definition, index, { checkpoints, runId: `bounded-${index}` })),
    );
    const coordinator = createWorkflowCoordinator({
      coordinatorId: "bounded-worker",
      workflows: { [definition.id]: definition },
      checkpoints,
      leases,
      maxConcurrentRuns: 2,
  ...
  it("takes over expired work and fences the stale worker", async () => {
    ...
    const stale = createWorkflowCoordinator({
      coordinatorId: "stale",
      workflows: { [definition.id]: definition },
      checkpoints,
      leases: losingLeases,
