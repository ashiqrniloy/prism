...
describe("observational memory runtime", () => {
  ...
  it("runtime_appends_custom_ledger_entries_append_only_and_redacted", async () => {
    ...
    const workerProvider = sequenceProvider([
      [
        providerToolCall(
          toolCallContent("o", "record_observation", {
            content: "secret-value package-only preference",
            relevance: "high",
      ...
      [
        providerToolCall(
          toolCallContent("r", "record_reflection", { content: "secret-value keep optional", supportingObservationIds: [] }),
        ),
        providerDone(),
      ],
    ]);
  ...
  it("runtime_rejects_legacy_store_option_and_wrong_append_owner", async () => {
    ...
    assert.equal(
      (await store.list(session.id)).some((entry) => entry.kind === "custom"),
      false,
    );
    assert.equal(
      (await otherStore.list(session.id)).some((entry) => entry.kind === "custom"),
      true,
    );
  });
